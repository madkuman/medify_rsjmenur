...
function initTransaksi(){
	...
	for(i=1;i<=countdetail;i++){
		...
		backboneAddTransaksiDetail(i,detail_id,layanan_id,nama,jumlah,harga,diskon,keterangan,subtotal_number,tipe,kelas,created_by,departemen_id);
	}
}
...
function initEditable_(){
	...
	$('.keterangan_').editable({
		...
		success: function(response, newValue) {
			...
			var id = $(this).data("pk")
			updateRecord(jumlah,harga,diskon,subtotal,id,keterangan,keterangan);
		}
	});
	...
	$('.kelas_').on('select2:select', function (e) {
		...
		updateRecord2(jumlah,harga,diskon,subtotal,id,keterangan,tipe,kelas,data2.tarif_id,data2.tarif_name,departemen_id);
...
function initEditable(){
	...
	$('.layanan').on('select2:select', function (e) {
		...
		backboneAddTransaksiDetail(id,null,data2.id,data2.deskripsi,1,'','','',subtotal_number,0,'',user,data2.departemen_id);
...
function updateRecord2(jumlah,harga,diskon,subtotal,id,keterangan,tipe,kelas,layanan_id,layanan_string,departemen_id) //update record w/ tipe, kelas, layanan id
{
	subtotal_number = (harga * jumlah) - (harga*diskon/100*jumlah);
	subtotal_number_formatted = numeral(subtotal_number).format('0,0');
	...
	backboneUpdateTransaksi2(jumlah,harga,diskon,subtotal_number,keterangan,id,tipe,kelas,layanan_id,layanan_string,departemen_id);
}
...
function backboneAddTransaksiDetail(id,detail_id,layanan_id,layanan_string,jumlah,harga,diskon,keterangan,subtotal,tipe,kelas,created_by,departemen_id)
{
...
function backboneUpdateTransaksi2(jumlah,harga,diskon,subtotal,keterangan,id,tipe,kelas,layanan_id,layanan_string,departemen_id)
{
	var transaksidetail = transaksiCollection.findWhere({pk_id: id});
	if(typeof transaksidetail !== "undefined") var exist = 1;
	else var exist = 0; 
...
$('#buttonSubmit').click(function() {
	...
	if(tanggal == '')
		...
		$.ajax({
			type: "POST",
			url: API_URL + "/keuangan/pemasukan/edit",
			dataType: "json",
			headers: {
				'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			},
