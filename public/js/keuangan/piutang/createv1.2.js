...
$('#pasien-pembayaran').on('select2:select', function (e) {
	...
	if($('#pasien').val() == null || pembayaran[0].text != 'Tunai')
		...
		document.getElementById("pihak3").value = pasien[0].name;
	}
...
$('#tambahTransaksi .js-autocomplete').autoComplete({
	...
	source: function(term, suggest){
		...
		$.ajax({
			url: API_URL+"/keuangan/tarif/search?keyword="+term+"&kelas="+kelas_id,
			type: 'GET',
			dataType: 'json',
			tryCount : 0,
			retryLimit : 3,
	...
	renderItem: function (item, search){
		...
		content = '<div class="autocomplete-suggestion border-bottom" data-harga="'+item['harga']+'"  data-id="'+item['id']+'" data-val="'+value+'">'
...
$('.kelas_id').on('select2:select', function (e) {
	...
	if(tarif_id != '')
	{
		eventUpdateKelasGetHarga(tarif_id,kelas_id);
	}
});
...
function eventUpdateKelasGetHarga(tarif_id,kelas_id)
{
	$('#tambahTransaksi #harga_loading').show();
	$.ajax({
		url: API_URL+"/keuangan/tarif/get-single?tarif="+tarif_id+"&kelas="+kelas_id,
		type: 'GET',
		dataType: 'json',
		tryCount : 0,
		retryLimit : 3,
...
function emptyInput()
{
	...
	//lokasi = $('#tambahTransaksi .lokasi').val(0).trigger('change');
	//tarif_kelas = $('#tambahTransaksi .kelas_id').val(0).trigger('change');
...
function showEditModal(data)
{
	...
	tarif_kelas_id = $('#tambahTransaksi .kelas_id').val(data.get('tarif_kelas_id')).trigger('change');
...
function updateAllTotal()
{
	...
	$.each(subtotals, function( index, value ) {
		...
		allDiskon+= parseInt(diskons[index])
	});
...
function backboneAddTransaksiDetail(data)
{
	...
	var detail = new TransaksiDetail({ 
		...
		tarif_kelas_id: data.tarif_kelas_id,
		jumlah: data.jumlah,
		harga: data.harga,
		diskon: data.diskon,
