...
})((typeof(fwd_module) !== "undefined" && fwd_module.exports && typeof(fwd_global) !== "undefined") ? fwd_global : this || window, "FWDAnimation");/* FWDButtonToolTip */
(function (window){
var FWDButtonToolTip = function(
		...
		self.setupMainContainers = function(){	
			...
			self.middle_sdo.getStyle().backgroundColor = "#000000";
...
var FWDController = function(
		...
		this.positionHideOrShowControllerButton = function(animate){
			...
			FWDAnimation.killTweensOf(self.hideShowControllerButton_do);
			if(animate){
				FWDAnimation.to(self.hideShowControllerButton_do, .8, {x:tempX, y:tempY, ease:Expo.easeInOut});
			}else{
		...
		self.setupBackground = function(){
			...
			middleImage.style.backgroundColor = "#000000";
	...
	var FWDData = function(props, playListElement){
		...
		self.parseProperties = function(){
			...
			if(!self.rootElement_el){
				errorMessage_str = "Make sure that the a div with the id - <font color='#FF0000'>" + self.props_obj.markersAndInfoWindowId + "</font> exists, self represents the data playlist.";
			...
			if(self.props_obj.buttons && self.props_obj.buttons.length > 0){
			...
			if(self.showNavigator_bl && !self.navigatorImagePath_str) {
				errorMessage_str = "The  <font color='#FF0000'>navigatorImagePath</font> is not defined in the contructor, this is necessary to setup the navigator.";
				self.dispatchEvent(FWDData.LOAD_ERROR, {text:errorMessage_str});
				return;
			}
			...
			self.disableMouseWheel_bl = self.props_obj.disableMouseWheel; 
			self.disableMouseWheel_bl = self.disableMouseWheel_bl == "no" ? false : true;
			...
			self.skinPaths_ar = [
				 ...
				 {img:self.controllerMoveDownN_img = new Image(), src:controllerDownButtonN_str},
				 {img:self.controllerMoveDownS_img = new Image(), src:controllerDownButtonS_str},
				 {img:self.controllerMoveDownD_img = new Image(), src:controllerDownButtonD_str},
				 {img:self.controllerMoveUpN_img = new Image(), src:controllerUpButtonN_str},
				 {img:self.controllerMoveUpS_img = new Image(), src:controllerUpButtonS_str},
				 {img:self.controllerMoveUpD_img = new Image(), src:controllerUpButtonD_str},
				 {img:self.controllerNextN_img = new Image(), src:controllerNextN_str},
				 {img:self.controllerNextS_img = new Image(), src:controllerNextS_str},
				 {img:self.controllerNextD_img = new Image(), src:controllerNextD_str},
				 {img:self.controllerPrevN_img = new Image(), src:controllerPrevN_str},
				 {img:self.controllerPrevS_img = new Image(), src:controllerPrevS_str},
				 {img:self.controllerPrevD_img = new Image(), src:controllerPrevD_str},
				 {img:self.controllerHideMarkersN_img = new Image(), src:hideMarkersN_str},
				 {img:self.controllerHideMarkersS_img = new Image(), src:hideMarkersS_str},
				 {img:self.controllerShowMarkersN_img = new Image(), src:showMarkersN_str},
				 {img:self.controllerShowMarkersS_img = new Image(), src:showMarkersS_str},
				 {img:self.controllerInfoN_img = new Image(), src:controllerInfoN_str},
				 {img:self.controllerHideN_img = new Image(), src:controllerHideN_str},
				 {img:self.controllerHideS_img = new Image(), src:controllerHideS_str},
				 {img:self.controllerShowN_img = new Image(), src:controllerShowN_str},
				 {img:self.controllerShowS_img = new Image(), src:controllerHideS_str},
				 {img:self.controllerFullScreenNormalN_img = new Image(), src:controllerFullScreemNormalN_str},
				 {img:self.controllerFullScreenNormalS_img = new Image(), src:controllerFullScreenNormalS_str},
				 {img:self.controllerFullScreenFullN_img = new Image(), src:controllerFullScreemFullN_str},
				 {img:self.controllerFullScreenFullS_img = new Image(), src:controllerFullScreenFullS_str},
				 {img:self.zoomInN_img = new Image(), src:zoomInN_str},
				 {img:self.zoomInS_img = new Image(), src:zoomInS_str},
				 {img:self.zoomOutN_img = new Image(), src:zoomOutN_str},
				 {img:self.zoomOutS_img = new Image(), src:zoomOutS_str},
				 {img:self.scrollBarHandlerN_img = new Image(), src:scrollBarHandlerN_str},
				 {img:self.scrollBarHandlerS_img = new Image(), src:scrollBarHandlerS_str},
				 {img:self.scrollBarLeft_img = new Image(), src:scrollBarLeft_str},
				 {img:self.scrollBarRight_img = new Image(), src:scrollBarRight_str},
				 ...
				 {img:self.infoWindowCloseNormal_img = new Image(), src:mainLightBoxCloseButtonNPath_str},
				 {img:self.infoWindowCloseSelected_img = new Image(), src:mainLightBoxCloseButtonNPath_str}
			 ]
		...
		self.onNavigatorImageLoadErrorHandler = function(e){
			var message = "The navigator image with the label <font color='#FF0000'>" + self.navigatorImagePath_str + "</font> can't be loaded, make sure that the image exists and the path is correct!";
		...
		self.onImageLoadErrorHandler = function(e){
			var message = "The image with the label <font color='#FF0000'>" + self.originalImagePath_str + "</font> can't be loaded, make sure that the image exists and the path is correct!";
			console.log(e);
			var err = {text:message};
			self.dispatchEvent(FWDData.LOAD_ERROR, err);
		};
		...
		self.showMarkerError = function(error, position){
			self.dispatchEvent(FWDData.LOAD_ERROR, {text:"The marker at position <font color='#FF0000'>" + (position + 1) + "</font> dose not have defined an attribute <font color='#FF0000'>" + error + "</font>."});
		};
...
var FWDDescriptionWindow = function(parent, data){
		...
		self.show = function(text){
			...
			if(FWDUtils.isMobile){
				FWDAnimation.to(self.background_sdo, .6, {alpha:self.backgroundOpacity, delay:.2});
		...
		self.showWithDelay = function(){
			...
			FWDAnimation.to(self.mainContentHolder_do, .6, {y:self.mainContentFinalY, ease:Expo.easeInOut});
		...
		self.hide = function(animate, overwrite){
			...
			if(animate){
				FWDAnimation.to(self.mainContentHolder_do, .6, {y:self.stageHeight, ease:Expo.easeInOut});
...
(function (window){
	...
	var FWDLightBox = function(
		...
		this.setupMainContainers = function(){
			...
			FWDAnimation.to(self.lightBoxBackground_sdo, 1, {delay:.1, alpha:self.lightBoxBackgroundOpacity});
...
var FWDMarker = function(
		...
		self.onMouseOver = function(e){
			...
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				...
				if(self.isHtml5_bl){
					self.n_do.setScale2(1);
					self.s_do.setScale2(1);
				}else{
					self.n_do.setWidth(self.width);
					self.n_do.setHeight(self.height);
					self.s_do.setWidth(self.width);
				...
				if(self.hasGif_bl){
					FWDAnimation.to(self.n_do, .5, {alpha:0, delay:.1, ease:Expo.easeOut});
				}else{
		...
		this.setNormalState = function(){
			FWDAnimation.killTweensOf(self.s_do);
			if(self.hasGif_bl){
				FWDAnimation.to(self.n_do, .5, {alpha:1, ease:Expo.easeOut});	
			}else{
		...
		self.show = function(){
			...
			if(self.isHtml5_bl){
				...
				self.s_do.setScale2(0);
				FWDAnimation.to(self.n_do, .8, {scale:1, delay:.1, ease:Elastic.easeOut});
				FWDAnimation.to(self.s_do, .8, {scale:1, delay:.1, ease:Elastic.easeOut});
			}else{
				self.n_do.setWidth(0);
				self.n_do.setHeight(0);
				self.s_do.setWidth(0);
				self.s_do.setHeight(0);
				FWDAnimation.to(self.n_do, .8, {w:self.width, h:self.height, delay:.1, ease:Elastic.easeOut});
				FWDAnimation.to(self.s_do, .8, {w:self.width, h:self.height, delay:.1, ease:Elastic.easeOut});
			}
		};
		...
		self.show = function(){
			...
			if(self.isMobile_bl){
				...
				if(FWDUtils.isIEAndLessThen9){
					...
					self.setAlpha(0);
					FWDAnimation.killTweensOf(self);
					FWDAnimation.to(self, .6, {alpha:1, delay:.1, ease:Quart.easeOut});
				}
			}
...
(function (window){
	...
	var FWDMegazoom = function(props){
		...
		self.skinPath_str = props.skinPath;
		
		if((self.skinPath_str.lastIndexOf("/") + 1) != self.skinPath_str.length){
			self.skinPath_str += "/";
		}
		
		this.warningIconPath_str = this.skinPath_str + "warning.png";
		...
		self.init = function(){
			...
			if(!self.props_obj.markersAndInfoWindowId){
			...
			this.rootElement_el = FWDUtils.getChildById(self.props_obj.markersAndInfoWindowId);
			...
			self.originalWidth = self.props_obj.zoomerWidth || 500;
			self.originalHeight = self.props_obj.zoomerHeight || 400;
		...
		self.resizeHandler = function(overwrite){
			...
			self.scrollOffsets = FWDUtils.getScrollOffsets();
			
			var viewportSize = FWDUtils.getViewportSize();
			
			if(self.viewportWidth == viewportSize.w && self.viewportHeight == viewportSize.h 
				&& self.pageXOffset == self.scrollOffsets.x && self.pageYOffset == self.scrollOffsets.y
				&& !overwrite) return;
			...
			if(self.displayType == FWDMegazoom.LIGHTBOX && !self.isFullScreen_bl){
				...
				self.lightBox_do.setX(self.scrollOffsets.x);
				self.lightBox_do.setY(self.scrollOffsets.y);
			...
			}else if(self.displayType == FWDMegazoom.RESPONSIVE){
				if (self.autoScale){
		...
		self.goFullScreen = function(){
			...
			self.lastScrollX = self.scrollOffsets.x;
			self.lastScrollY = self.scrollOffsets.y;
...
(function (window){
	 ...
	 */
	var FWDTransformDisplayObject = function(type, position, overflow, display){
		...
		this.setTransformOrigin = function(x, y){
			self.screen.style[self.getTransformOrigin()] = x + "%" + " " +  y + "%";	
		};
