...
class IndeksTindakan implements FromView, WithEvents
{
	use Exportable;
	...
	public function registerEvents(): array
	{
		return [
			AfterSheet::class    => function(AfterSheet $event) {
				...
				$start_row = 4;
				foreach($this->data['tindakan'] as $item)
				{
					...
					{
						$event->sheet->styleCells(
							'A'.$start_table_header.':AC'.($start_row_content + count($item->kasus) - 1),
							[
								'borders' => [
									'allBorders' => [
						...
						$event->sheet->styleCells(
							'A'.$start_table_header.':AC'.($start_table_header + 2),
							[
								'font' => [
									'bold' => true,
								],
	...
	{
		return view ('pasien.laporan.indeks-tindakan',$this->data);
	}
}
