...
class LaporanAktifitasPoliPsikologiExcel implements FromView, WithEvents, WithColumnFormatting
{
    ...
    public function __construct($data)
    {
        $count_column = 5;
        $d = $data['data'];
        $count_row = count($d['transaksi']) + count($d['tindakan']) +  count($d['rujukan']);
        $this->data = $data;
        $this->last_column = excel_column($count_column);
        $this->last_row = $count_row + 13;
    ...
    {
        return view('pasien.laporan.laporan-aktifitas-poli-psikologi', $this->data);
    }
