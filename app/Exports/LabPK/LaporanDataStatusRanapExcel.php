...
class LaporanDataStatusRanapExcel implements FromView, WithEvents
{
    ...
    {
        $count_column = 1;
        foreach($data['data'][0] as $key=> $data_item){
            if($key != 'lokasi'){
                $count_column+=count($data_item);
            }
        }
        $count_column+=2;
    ...
    {
        return view('labpk.laporan.view.laporan-data-status-ranap', $this->data);
    }
