...
use Carbon\Carbon;
use App\Exports\Radiologi\RekapPemeriksaanPasienHarian;
use File;

class RekapPemeriksaanPasienHarianGenerate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'radiologi:laporan-rekap-pemeriksaan-pasien-harian {date=0}';

    /**
     * The console command description.
     *
     * @var string
     */ protected $description = 'Generate Laporan Radiologi Rekap HARIAN Format : dd-mm-yyyy';
    ...
    public function handle()
    {
        ...
        $base_path = '/downloads/laporan/radiologi/rekap-pemeriksaan-pasien-harian';
        $path = public_path().$base_path;
        $path_download = url('/').$base_path;
       
        $data['data'] = app('App\Http\Controllers\Radiology\Laporan\ReadRekapHarianController')->get($start,$end);

        $data['tanggal'] = $start->copy()->format('d F Y');

        $filename = 'Rekap Pemeriksaan Pasien Harian - '.$start->copy()->format('Y-m-d');
        ...
        $xls = (new RekapPemeriksaanPasienHarian($data))->store($filename_stored);
        File::move(storage_path('app/'.$filename_stored), $path.'/'.$filename_stored);

        $laporan_data = [];
        $laporan_data['slug'] = 'radiologi-rekap-pemeriksaan-pasien-harian';
