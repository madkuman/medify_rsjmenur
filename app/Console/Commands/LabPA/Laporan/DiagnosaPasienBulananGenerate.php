...
use Carbon\Carbon;
use App\Exports\LabPA\DataDiagnosaPasienBulanan;
use File;

class DiagnosaPasienBulananGenerate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'labpa:laporan-diagnosa-pasien-bulanan {date=0}';
     ...
     * @var string
     */
    protected $description = 'Generate Data Lab PA Diagnosa Bulanan Format : YYYY-MM (2020-04)';
    ...
    public function handle()
    {
        ...
        $base_path = '/downloads/laporan/labpa/data-diagnosa-pasien-bulanan';
        $path = public_path().$base_path;
        $path_download = url('/').$base_path;
       
        $data = app('App\Http\Controllers\LabPA\Laporan\ReadDataDiagnosaPasienBulananController')->get($start,$end);

        $data['bulan'] = $start->copy()->format('F Y');

        $filename = 'Data Diagnosa Pasien Bulanan - '.$start->copy()->format('Y-m-d');
        $timestamp_now = Carbon::now()->timestamp;
        $format = '.xlsx';

        $filename_stored = $filename.'-'.$timestamp_now.$format;

        $xls = (new DataDiagnosaPasienBulanan($data))->store($filename_stored);
        File::move(storage_path('app/'.$filename_stored), $path.'/'.$filename_stored);

        $laporan_data = [];
        $laporan_data['slug'] = 'lab-pa-data-diagnosa-pasien-bulanan';
