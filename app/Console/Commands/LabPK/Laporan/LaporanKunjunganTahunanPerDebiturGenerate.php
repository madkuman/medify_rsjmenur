...
use Carbon\Carbon;
use File;
use App\Exports\LabPK\LaporanKunjunganTahunanPerDebiturExcel;

class LaporanKunjunganTahunanPerDebiturGenerate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'labpk:laporan-kunjungan-tahunan-per-debitur {date=0}';
     ...
     * @var string
     */
    protected $description = 'Generate Laporan Kunjungan Tahunan Per Debitur Tahunan : YYYY-MM (2020-04)';
    ...
    public function handle()
    {
        ...
        $base_path = '/downloads/laporan/labpk/laporan-kunjungan-tahunan-per-debitur';
        ...
        $filename = 'Laporan Kunjungan Tahunan Per Debitur - '.$end->copy()->format('Y-m');
        $timestamp_now = Carbon::now()->timestamp;
        $format = '.xlsx';

        $filename_stored = $filename.'-'.$timestamp_now.$format;

        $xls = (new LaporanKunjunganTahunanPerDebiturExcel($data))->store($filename_stored);
        ...
        $laporan_data = [];
        $laporan_data['slug'] = 'lab-pk-laporan-kunjungan-tahunan-per-debitur';
