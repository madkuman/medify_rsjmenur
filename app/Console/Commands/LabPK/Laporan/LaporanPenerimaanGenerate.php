...
class LaporanPenerimaanGenerate extends Command
{
     ...
     * @var string
     */
    protected $signature = 'labpk:laporan-penerimaan {date=0}';
     ...
     * @var string
     */
    protected $description = 'Generate Laporan Penerimaan Bulanan Format : YYYY-MM (2020-04)';
    ...
    public function handle()
    {
        ...
        $data['data'] = app('App\Http\Controllers\LabPK\Laporan\ReadLaporanPenerimaanController')->get($start,$end);

        $data['start_date'] = $start->copy();
        $data['end_date'] = $end->copy();


        $filename = 'Laporan Penerimaan - '.$end->copy()->format('Y-m');
