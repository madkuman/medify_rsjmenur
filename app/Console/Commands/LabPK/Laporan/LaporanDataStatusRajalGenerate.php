...
use Carbon\Carbon;
use File;
use App\Exports\LabPK\LaporanDataStatusRajalExcel;

class LaporanDataStatusRajalGenerate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'labpk:laporan-data-status-rajal {date=0}';
     ...
     * @var string
     */
    protected $description = 'Generate Laporan Data Status Rajal Bulanan Format : YYYY-MM (2020-04)';
    ...
    public function handle()
    {
        ...
        $base_path = '/downloads/laporan/labpk/laporan-data-status-rajal';
        $path = public_path().$base_path;
        $path_download = url('/').$base_path;

        $data = app('App\Http\Controllers\LabPK\Laporan\ReadLaporanDataStatusRajalController')->get($start,$end);

        $data['bulan'] = indonesian_date($end->copy(),'F Y');


        $filename = 'Laporan Data Status Rajal - '.$end->copy()->format('Y-m');
        $timestamp_now = Carbon::now()->timestamp;
        $format = '.xlsx';

        $filename_stored = $filename.'-'.$timestamp_now.$format;

        $xls = (new LaporanDataStatusRajalExcel($data))->store($filename_stored);
        ...
        $laporan_data = [];
        $laporan_data['slug'] = 'lab-pk-laporan-data-status-rajal';
