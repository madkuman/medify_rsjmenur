...
use Carbon\Carbon;
use File;
use App\Exports\LabPK\LaporanKunjunganTahunanPerLokasiExcel;

class LaporanKunjunganTahunanPerLokasiGenerate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'labpk:laporan-kunjungan-tahunan-per-lokasi {date=0}';
     ...
     * @var string
     */
    protected $description = 'Generate Laporan Kunjungan Tahunan Per Lokasi Tahunan : YYYY-MM (2020-04)';
    ...
    public function handle()
    {
        ...
        $base_path = '/downloads/laporan/labpk/laporan-kunjungan-tahunan-per-lokasi';
        $path = public_path().$base_path;
        $path_download = url('/').$base_path;
       
        $data['data'] = app('App\Http\Controllers\LabPK\Laporan\ReadLaporanKunjunganTahunanPerLokasiController')->get($start,$end);

        $data['start_date'] = $start->copy();
        $data['end_date'] = $end->copy();


        $filename = 'Laporan Kunjungan Tahunan Per Lokasi - '.$end->copy()->format('Y-m');
        $timestamp_now = Carbon::now()->timestamp;
        $format = '.xlsx';

        $filename_stored = $filename.'-'.$timestamp_now.$format;

        $xls = (new LaporanKunjunganTahunanPerLokasiExcel($data))->store($filename_stored);
        ...
        $laporan_data = [];
        $laporan_data['slug'] = 'lab-pk-laporan-kunjungan-tahunan-per-lokasi';
