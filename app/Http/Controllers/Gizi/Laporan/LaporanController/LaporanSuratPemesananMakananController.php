...
class LaporanSuratPemesananMakananController extends Controller
{
    public function get($request)
    {
        ...
        //     foreach ($new_kelas as $kelas) {
        //         foreach ($kelas['bangsal_ids'] as $bangsal_id) {
        //             $data_tambahan[$jenis->nama][] = PemesananDetail::whereBetween('untuk_tanggal', [$date_start, $date_end])->where('bangsal_id', $bangsal_id)->whereIn('kelas_id', $kelas['kelas_ids'])->whereRaw("JSON_CONTAINS(makanan_tambahan_ids, ?)", [json_encode($jenis->id)])->where('waktu_makan_id', $waktu_makan->id)->where('gender', 1)->count();
        //             $data_tambahan[$jenis->nama][] = PemesananDetail::whereBetween('untuk_tanggal', [$date_start, $date_end])->where('bangsal_id', $bangsal_id)->whereIn('kelas_id', $kelas['kelas_ids'])->whereRaw("JSON_CONTAINS(makanan_tambahan_ids, ?)", [json_encode($jenis->id)])->where('waktu_makan_id', $waktu_makan->id)->where('gender', 2)->count();
        //         }
        //     }
        //     $data_tambahan[$jenis->nama][] = PemesananDetail::whereBetween('untuk_tanggal', [$date_start, $date_end])->whereRaw("JSON_CONTAINS(makanan_tambahan_ids, ?)", [json_encode($jenis->id)])->where('waktu_makan_id', $waktu_makan->id)->where('gender', 1)->count();
        //     $data_tambahan[$jenis->nama][] = PemesananDetail::whereBetween('untuk_tanggal', [$date_start, $date_end])->whereRaw("JSON_CONTAINS(makanan_tambahan_ids, ?)", [json_encode($jenis->id)])->where('waktu_makan_id', $waktu_makan->id)->where('gender', 2)->count();
        //     $data_tambahan[$jenis->nama][] = PemesananDetail::whereBetween('untuk_tanggal', [$date_start, $date_end])->whereRaw("JSON_CONTAINS(makanan_tambahan_ids, ?)", [json_encode($jenis->id)])->where('waktu_makan_id', $waktu_makan->id)->count();
        // }
        // dd($data_tambahan);
