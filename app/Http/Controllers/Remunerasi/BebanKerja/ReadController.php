...
class ReadController extends Controller
{
    ...
    function index(Request $request){
        ...
        $beban_kerja = BebanKerja::query();

        // filter by periode 
        if(!empty($periode)){
            $beban_kerja = $beban_kerja->where('bulan', 'like', "%{$periode}%");
            $total_filter = count(with(clone $beban_kerja)->get());
            $total_record = $total_filter;
        }
        ...
        if(!empty($cari)){
            ...
            if(!empty($pegawai_ids)) {
                ...
                $beban_kerja = $beban_kerja->whereIn('pegawai_id', $pegawai_ids)->orderByRaw("FIELD(pegawai_id, $pegawai_ids_implode)");
            }
            $total_filter = count(with(clone $beban_kerja)->get());
            $total_record = $total_filter;
        }
        $beban_kerja = $beban_kerja->with('pegawai')->offset($start)->limit($limit)->get();
        $data = [];
        $no = $start+1;
        foreach ($beban_kerja as $row) {
            ...
            $value['action']    = '<td class="">
                        <a onclick="editBeban(this)" data-id="'.$row->id.'" data-bulan="'.date_format($bulan,"M-Y").'" data-nama="'.$row->pegawai->name.'" data-nrp="'.$row->pegawai->nrp.'" data-beban="'.$row->index.'" class="btn btn-sm btn-outline-info mr-5 mb-5 btn-keuangan"><i class="fa fa-pencil"></i></a></td>';
    ...
    {
        $beban_kerja = BebanKerja::where('pegawai_id',$pegawai_id)->where('bulan',$bulan_tahun)->first();
        return $beban_kerja;
    }

    public function getByTanggal($tanggal)
    {
        $beban_kerja = BebanKerja::where('bulan',$tanggal)->get();
        return $beban_kerja;
    }
