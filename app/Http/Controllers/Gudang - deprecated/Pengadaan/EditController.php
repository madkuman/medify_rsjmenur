    ...
    {   
        ...
        try
        {
            ...
            return redirect()->back()
                      ->with('message', 'Tidak dapat mengubah penerimaan. PJK telah diproses.')
                      ->with('status', -1)
                      ->with('title', 'Gagal');
            }
            ...
            if($referral_number != $transaction->nomor_referensi)
                $is_ganti_utang = true;
            ...
            foreach($transaction->log as $log)
            {
                ...
                {
                    if($log->id == $refer[$i]) {
                        $subtotal = app('App\Http\Controllers\Gudang\LogPengadaan\EditController')->edit($refer[$i],$items[$i],$qty[$i],$harga[$i],$diskon[$i],$ppn[$i],$subzero[$i],$expired[$i],$date,$batch[$i]);
                        $total += $subtotal;
                        $i++;
                        array_push($new_processed_jumlah, $qty[$i] - $log->jumlah);
                        array_push($new_processed_subtotal, $subtotal[$i] - $log->subtotal);
                    }
                    else{
                      app('App\Http\Controllers\Gudang\LogPengadaan\DeleteController')->deleteLog($log->id);
                        array_push($new_processed_jumlah, - $log->jumlah);
                        array_push($new_processed_subtotal, - $log->subtotal);
                    }
                }
                else {
                    app('App\Http\Controllers\Gudang\LogPengadaan\DeleteController')->deleteLog($log->id);
            ...
            foreach($items as $item)
            {
                ...
                {
                    if(!is_null($qty[$j])) {
                        $log = app('App\Http\Controllers\Gudang\Items\CreateController')->newItem($item,$qty[$j],$expired[$j]);
                        $subtotal = app('App\Http\Controllers\Gudang\LogPengadaan\CreateController')->create($log,$qty[$j],$transaction->id,$harga[$j],$diskon[$j],$ppn[$j],$subzero[$j],$date,$batch[$i]);
                     

                        if(isset($po) && isset( $po->detail[$j])){
                            if($is_ganti_utang){
                                $detail_utang['jumlah'] = $qty[$j];
                                $detail_utang['subtotal'] = $subtotal;
                            }
                            else{
                                $detail_utang['jumlah'] = $new_processed_jumlah[$j] + (float) $qty[$j];
                                $detail_utang['subtotal'] = $new_processed_subtotal[$j] + $subtotal;
                                $subtotal = $new_processed_subtotal[$j] + $subtotal;
                            }
            ...
            // dd($total, $total_non_diskon, $diskon_total, $detail_utangs);
