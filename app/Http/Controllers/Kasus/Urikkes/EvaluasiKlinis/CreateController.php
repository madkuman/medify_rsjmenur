...
class CreateController extends Controller
{
    public function index(Request $request, $nomor_kasus){
      ...
      try {
        ...
        //$eval->bentuk_badan = $request->input('bentuk-badan');
        ...
        $eval->save();
        $identitas->save();
        app('App\Http\Controllers\Kasus\Urikkes\Mata\CreateController')->mata($request,$nomor_kasus,$eval->id);
        app('App\Http\Controllers\Kasus\Urikkes\Telinga\CreateController')->create($request,$nomor_kasus,$eval->id);
        DB::connection('kasus')->commit();
        
        $status = 1;
        $message = 'Evaluasi baru berhasil dibuat!';
        $title = 'Berhasil!';
