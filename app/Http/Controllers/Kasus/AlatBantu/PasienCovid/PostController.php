...
class PostController extends Controller
{
	public function save($nomor_kasus,Request $request)
	{
		...
		if(!empty($covid->kontak) && !empty($covid->bapil) && empty($covid->demam) && empty($covid->nafas))
			$alatBantu->presentase = 3;
		elseif(	
			(!empty($covid->demam) && !empty($covid->bapil) && (!empty($covid->daerah) || !empty($covid->negara))) || 
			(!empty($covid->kontak) && (!empty($covid->demam) || !empty($covid->bapil))) || 
			!empty($covid->nafas)
		)
    	...
    	if($alatBantu->presentase == 1) $status = 'pdp';
    	elseif($alatBantu->presentase == 2) $status = 'otg';
    	elseif($alatBantu->presentase == 3) $status = 'odp';
    	elseif($alatBantu->presentase == 4) $status = 'positif';
    	elseif($alatBantu->presentase == 5) $status = 'probable';
    	elseif($alatBantu->presentase == 0) $status = 'negatif';
