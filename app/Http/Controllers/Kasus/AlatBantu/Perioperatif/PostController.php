...
{
	static protected $type = "perioperatif";
	...
	{	
		...
		try {
			...
			else
			{
				...
				$status = -1;
				$message = 'Asesmen perioperatif gagal dibuat!';
				$title = 'Error!';
	...
	public function create($kasus_id, $content)
	{
		...
		$status = 1;
		$message = 'Asesmen Perioperatif berhasil dibuat';
		$title = 'Berhasil!';

		$log = app('App\Http\Controllers\Kasus\Log\CreateController')
		->create($kasus_id,'create','alat-perioperatif',$alatBantu->id);
		return [
			'status' => $status,
			'message' => $message,
			'title' => $title
	...
	public function update($id, $kasus_id, $content)
	{
		...
		$status = 1;
		$message = 'Asesmen Perioperatif berhasil diupdate';
		$title = 'Berhasil!';

		$log = app('App\Http\Controllers\Kasus\Log\CreateController')
		->create($kasus_id,'update','alat-perioperatif',$alatBantu->id);
		return [
			'status' => $status,
			'message' => $message,
			'title' => $title
	...
	{	
		try {
			...
			$status = 1;
			$message = 'Asesmen Perioperatif berhasil dihapus';
			$title = 'Berhasil!';
