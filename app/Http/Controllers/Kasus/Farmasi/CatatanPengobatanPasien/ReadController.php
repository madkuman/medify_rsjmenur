...
class ReadController extends Controller
{
	public function checkIfExist($kasus_id,$nama_obat, $obat_id, $aturan){
		...
		$count = CatatanPengobatanPasien::where('kasus_id',$kasus_id)->where('nama_obat',$nama_obat)->get();

		if(count($count) == 0)
		{
			$count = CatatanPengobatanPasien::where('kasus_id',$kasus_id)->where('obat_id',$obat_id)->get();
