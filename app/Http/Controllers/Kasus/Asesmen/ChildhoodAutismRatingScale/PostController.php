...
namespace App\Http\Controllers\Kasus\Asesmen\ChildhoodAutismRatingScale;;
...
class PostController extends Controller
{
    ...
    {
        $this->asesmen_title = 'CARS (Childhood Autism Rating Scale)';
        $this->asesmen_namespace = 'App\Http\Controllers\Kasus\Asesmen\ChildhoodAutismRatingScale;';
        $this->asesmen_slug = 'childhood-autism-rating-scale';
    }
    ...
    public function submitForm($nomor_kasus, Request $request)
    {
        ...
        try {
            ...
            foreach ($input as $key => $val) {
                ...
                if ($key == 'hubungan_oranglain') $hasil[$key] = $val;
                if ($key == 'meniru') $hasil[$key] = $val;
                if ($key == 'respon_emosi') $hasil[$key] = $val;
                if ($key == 'penggunaan_badan') $hasil[$key] = $val;
                if ($key == 'penggunaan_objek') $hasil[$key] = $val;
                if ($key == 'adaptasi_perubahan') $hasil[$key] = $val;
                if ($key == 'respon_visual') $hasil[$key] = $val;
                if ($key == 'respon_mendengar') $hasil[$key] = $val;
                if ($key == 'respon_perasa') $hasil[$key] = $val;
                if ($key == 'nervous') $hasil[$key] = $val;
                if ($key == 'komunikasi_verbal') $hasil[$key] = $val;
                if ($key == 'komunikasi_nonverbal') $hasil[$key] = $val;
                if ($key == 'level_aktivitas') $hasil[$key] = $val;
                if ($key == 'respon_intelektual') $hasil[$key] = $val;
                if ($key == 'kesan_umum') $hasil[$key] = $val;
    ...
    public function addTTD($nomor_kasus = null, Request $request)
    {
        ...
        try {
            ...
            if (!is_null($nomor_kasus)) {
                $url = 'kasus/' . $nomor_kasus . '/asesmen/checklist-autisme-toddler';
            } else {
