...
namespace App\Http\Controllers\Kasus\Asesmen\SelfHarmInventory;;
...
class PostController extends Controller
{
    ...
    {
        $this->asesmen_title = 'SHI (Self Harm Inventory)';
        $this->asesmen_namespace = 'App\Http\Controllers\Kasus\Asesmen\SelfHarmInventory;';
        $this->asesmen_slug = 'self-harm-inventory';
    }
    ...
    public function submitForm($nomor_kasus, Request $request)
    {
        ...
        try {
            ...
            foreach ($input as $key => $val) {
                ...
                if ($key == 'overdosis') $hasil[$key] = $val;
                if ($key == 'melukai_diri') $hasil[$key] = $val;
                if ($key == 'membakar_diri') $hasil[$key] = $val;
                if ($key == 'memukul_diri') $hasil[$key] = $val;
                if ($key == 'membentur_kepala') $hasil[$key] = $val;
                if ($key == 'penyalahgunaan_alkohol') $hasil[$key] = $val;
                if ($key == 'berkendara_ugal') $hasil[$key] = $val;
                if ($key == 'menggores_diri') $hasil[$key] = $val;
                if ($key == 'mencegah_sembuhluka') $hasil[$key] = $val;
                if ($key == 'sengaja_jadiburuk') $hasil[$key] = $val;
                if ($key == 'sengaja_banyakpasangan') $hasil[$key] = $val;
                if ($key == 'menolak_hubungan') $hasil[$key] = $val;
                if ($key == 'penyalahgunaan_resep') $hasil[$key] = $val;
                if ($key == 'menjauh_tuhan') $hasil[$key] = $val;
                if ($key == 'kekerasan_hubungan') $hasil[$key] = $val;
                if ($key == 'kekerasan_seks') $hasil[$key] = $val;
                if ($key == 'kehilangan_pekerjaan') $hasil[$key] = $val;
                if ($key == 'coba_bundir') $hasil[$key] = $val;
                if ($key == 'sengaja_cederadiri') $hasil[$key] = $val;
                if ($key == 'hancur_diri') $hasil[$key] = $val;
                if ($key == 'menahan_lapar') $hasil[$key] = $val;
                if ($key == 'minum_obatpencahar') $hasil[$key] = $val;
    ...
    public function addTTD($nomor_kasus = null, Request $request)
    {
        ...
        try {
            ...
            if (!is_null($nomor_kasus)) {
                $url = 'kasus/' . $nomor_kasus . '/asesmen/self-harm-inventory';
            } else {
