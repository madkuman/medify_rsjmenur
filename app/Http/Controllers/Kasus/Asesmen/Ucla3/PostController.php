...
namespace App\Http\Controllers\Kasus\Asesmen\Ucla3;;
...
class PostController extends Controller
{
    ...
    {
        $this->asesmen_title = 'UCLA-3';
        $this->asesmen_namespace = 'App\Http\Controllers\Kasus\Asesmen\Ucla3;';
        $this->asesmen_slug = 'ucla-3';
    }
    ...
    public function submitForm($nomor_kasus, Request $request)
    {
        ...
        try {
            ...
            foreach ($input as $key => $val) {
                ...
                if ($key == 'tidakcocok_orangsekitar') $hasil[$key] = $val;
                if ($key == 'tidakpunya_teman') $hasil[$key] = $val;
                if ($key == 'tidakdapat_mintatolong') $hasil[$key] = $val;
                if ($key == 'merasa_sendiri') $hasil[$key] = $val;
                if ($key == 'bagian_kelompok') $hasil[$key] = $val;
                if ($key == 'banyak_persamaan') $hasil[$key] = $val;
                if ($key == 'tidakdekat_oranglain') $hasil[$key] = $val;
                if ($key == 'hobi_tidaksama') $hasil[$key] = $val;
                if ($key == 'merasa_ramah') $hasil[$key] = $val;
                if ($key == 'merasa_dekat') $hasil[$key] = $val;
                if ($key == 'merasa_ditinggalkan') $hasil[$key] = $val;
                if ($key == 'hubungan_tidakberarti') $hasil[$key] = $val;
                if ($key == 'tidakadaorang_mengenal') $hasil[$key] = $val;
                if ($key == 'terisolasi') $hasil[$key] = $val;
                if ($key == 'menemukan_teman') $hasil[$key] = $val;
                if ($key == 'mengerti_anda') $hasil[$key] = $val;
                if ($key == 'merasa_malu') $hasil[$key] = $val;
                if ($key == 'tidakbersama_anda') $hasil[$key] = $val;
                if ($key == 'ngobrol') $hasil[$key] = $val;
                if ($key == 'adaorang_yangtolong') $hasil[$key] = $val;
    ...
    public function addTTD($nomor_kasus = null, Request $request)
    {
        ...
        try {
            ...
            if (!is_null($nomor_kasus)) {
                $url = 'kasus/' . $nomor_kasus . '/asesmen/ucla-3';
            } else {
