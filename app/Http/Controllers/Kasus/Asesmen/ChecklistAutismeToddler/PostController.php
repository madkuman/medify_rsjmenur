...
namespace App\Http\Controllers\Kasus\Asesmen\ChecklistAutismeToddler;;
...
class PostController extends Controller
{
    ...
    {
        $this->asesmen_title = 'CHAT (Checlist for Autisme in Toddler)';
        $this->asesmen_namespace = 'App\Http\Controllers\Kasus\Asesmen\ChecklistAutismeToddler;';
        $this->asesmen_slug = 'checklist-autisme-toddler';
    }
    ...
    public function submitForm($nomor_kasus, Request $request)
    {
        ...
        try {
            ...
            foreach ($input as $key => $val) {
                ...
                if ($key == 'ayunan_naikturun') $hasil[$key] = $val;
                if ($key == 'tertarik_anaklain') $hasil[$key] = $val;
                if ($key == 'cilukba') $hasil[$key] = $val;
                if ($key == 'memanjat') $hasil[$key] = $val;
                if ($key == 'bermain_cangkir') $hasil[$key] = $val;
                if ($key == 'lihat_kesana') $hasil[$key] = $val;
                if ($key == 'menunjuk_jari') $hasil[$key] = $val;
                if ($key == 'mainan_kecil') $hasil[$key] = $val;
                if ($key == 'tunjuk_benda') $hasil[$key] = $val;
