...
namespace App\Http\Controllers\Kasus\Asesmen\BarthelIndex;;
...
class PostController extends Controller
{
    ...
    {
        $this->asesmen_title = 'BARTHEL INDEX';
        $this->asesmen_namespace = 'App\Http\Controllers\Kasus\Asesmen\BarthelIndex;';
        $this->asesmen_slug = 'barthel-index';
    }
    public function submitForm($nomor_kasus, Request $request)
    {
        ...
        try {
            ...
            foreach ($input as $key => $val) {
                ...
                if ($key == 'makan') $hasil[$key] = $val;
                if ($key == 'mandi') $hasil[$key] = $val;
                if ($key == 'personal_hygiene') $hasil[$key] = $val;
                if ($key == 'berpakaian') $hasil[$key] = $val;
                if ($key == 'bab') $hasil[$key] = $val;
                if ($key == 'bak') $hasil[$key] = $val;
                if ($key == 'toilet') $hasil[$key] = $val;
                if ($key == 'berpindah_tt') $hasil[$key] = $val;
                if ($key == 'mobilisasi') $hasil[$key] = $val;
                if ($key == 'turun_tangga') $hasil[$key] = $val;
    ...
    public function addTTD($nomor_kasus = null, Request $request)
	 {
		...
		try {
			...
			if (!is_null($nomor_kasus)) {
				$url = 'kasus/' . $nomor_kasus . '/asesmen/barthel-index';
			}else{
