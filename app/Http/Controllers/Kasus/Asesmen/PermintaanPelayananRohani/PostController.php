...
namespace App\Http\Controllers\Kasus\Asesmen\PermintaanPelayananRohani;;
...
class PostController extends Controller
{
    ...
    {
        $this->asesmen_title = 'Permintaan Pelayanan Rohani';
        $this->asesmen_namespace = 'App\Http\Controllers\Kasus\Asesmen\PermintaanPelayananRohani;';
        $this->asesmen_slug = 'permintaan-pelayanan-rohani';
    }
        ...
        try {
            ...
            if ($id != null) {
                return redirect('kasus/' . $nomor_kasus . '/asesmen/' . $this->asesmen_slug . '/edit/' . $id)
                    ->with('status', 1)
                    ->with('title', 'Sukses')
                    ->with('message', 'Formulir Permintaan Pelayanan Rohani Berhasil Diperbaharui!');
            } else {
                return redirect('kasus/' . $nomor_kasus . '/asesmen/' . $this->asesmen_slug)
                    ->with('status', 1)
                    ->with('title', 'Sukses')
                    ->with('message', 'Formulir Permintaan Pelayanan Rohani Berhasil Dibuat!');
            }
        } catch (\Exception $e) {
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            return back()
                ->with('status', 1)
                ->with('title', 'Sukses')
                ->with('message', 'Formulir Permintaan Pelayanan Rohani Berhasil di Hapus');
        } catch (\Exception $e) {
            DB::connection('kasus')->rollBack();
            $data['type'] = 'error';
            $data['title'] = 'Gagal';
    ...
    public function addTTD($nomor_kasus = null, Request $request)
	 {
		...
		try {
			...
			if (!is_null($nomor_kasus)) {
				$url = 'kasus/' . $nomor_kasus . '/asesmen/permintaan-pelayanan-rohani';
			}else{
