...
class ReadController extends Controller
{
    ...
    public function search($request)
    {
        ...
        if(!empty($search)) {
            if($jenis) {
                $item = ItemsTemplate::search($search)->rule(\App\SearchRule\FarmasiItems::class)->where('jenis',$jenis)->paginate(20);
            }
            else $item = ItemsTemplate::search($search)->rule(\App\SearchRule\FarmasiItems::class)->paginate(20);
        }
        else
    ...
    public function getItems($farmasi, Request $request)
    {
        ...
        if(!empty($search))
        {
            ...
            $arrStr = implode(',', $arr);
            $item = ItemsFarmasi::whereIn('item_template_id',$arr)->where('farmasi_id', $farm->id)->orderByRaw(DB::raw("FIELD(item_template_id, $arrStr)"))->with('item_detail', 'kadal', 'stok')->paginate(10);
            if(!empty($farm_asal))
            $item_asal = ItemsFarmasi::whereIn('item_template_id',$arr)->where('farmasi_id', $farm_asal->id)->orderByRaw(DB::raw("FIELD(item_template_id, $arrStr)"))->with('item_detail', 'kadal', 'stok')->paginate(10);
    ...
    public function getKartuStok($item,$tgl_awal,$tgl_akhir)
    {
        ...
        return ['stok_awal' => $stok_awal, 'riwayat' => $riwayat];
    ...
    public function getNarkotika($farmasi_id,$tgl_awal,$tgl_akhir,$kategori)
    {
        ...
        if($kategori) $id = ItemsTemplate::whereHas('kategori_item', function($cat) use($kategori){
                    $cat->whereIn('kategori_id', $kategori);
                })->orderBy('satuan', 'asc')->orderBy('nama', 'asc')->pluck('id');
        else $id = ItemsTemplate::orderBy('satuan', 'asc')->orderBy('nama', 'asc')->pluck('id');
        ...
        foreach($items as $item) {
            ...
            if($item->stok_awal || $item->masuk) array_push($arr,$item);
        }
        return $arr;
    }
    ...
    public function getPemakaian($farmasi_id,$tgl_awal,$tgl_akhir,$mode,$kategori,$jenis,$shift, $array_pembayaran)
    {
        ...
        if($mode == 1) $isBebas = "AND transaksi_obat.kasus_id IS NOT NULL";
        elseif($mode == 2) $isBebas = "AND transaksi_obat.kasus_id IS NOT NULL";
        else $isBebas = "";
    ...
    public function getPengeluaran($farmasi_id,$tgl_awal,$tgl_akhir,$mode)
    {
        ...
        foreach($items as $item) {
            if($mode)
            {
                if($mode == 1) $trans = ResepDetail::whereBetween('created_at', [$min_date, $max_date])->where('obat_id', $item->id)->whereHas('log')->whereHas('resep_detail', function($res){
                        $res->whereHas('transaksi_detail', function($tra){
                            $tra->whereNull('kasus_id');
                        });
                    })->get();
                else $trans = ResepDetail::whereBetween('created_at', [$min_date, $max_date])->where('obat_id', $item->id)->whereHas('log')->whereHas('resep_detail', function($res){
                        $res->whereHas('transaksi_detail', function($tra){
                            $tra->whereNotNull('kasus_id');
                        });
                    })->get();
            }
            else $trans = ResepDetail::whereBetween('created_at', [$min_date, $max_date])->where('obat_id', $item->id)->whereHas('log')->get();
            ...
            if($item->jumlah) array_push($arr,$item);
        }
    ...
    public function getPemberian($farmasi_id,$tgl_awal,$tgl_akhir,$pasien_id)
    {
        ...
        if(!$farmasi_id) $farmasi_id = Farmasi::pluck('id');
    ...
    public function getPemberianPerBangsal($farmasi_id,$bangsal_id,$tgl_awal,$tgl_akhir,$pasien_id)
    {
        ...
        if(!$farmasi_id) $farmasi_id = Farmasi::pluck('id');
        if(!$bangsal_id) $bangsal_id = Bangsal::pluck('id');
        ...
        if($lokasi) $transaksi = $transaksi->whereIn('lokasi_id',$lokasi);
    ...
    public function mutasiStokQueryWithNewFilter($params)
    {
        ...
        $query_sumber_dana = '';
        if ($sumber_dana_id != 0) $query_sumber_dana = "AND d.sumber_dana_id = $sumber_dana_id";
