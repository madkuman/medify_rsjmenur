...
class LaporanPenerimaanBarangHabisPakaiController extends Controller
{
    public function get($request,$item_template_ids,$supplier_ids)
    {
        ...
        $data['data'] = Pengadaan::whereBetween('created_at',[$date_start,$date_end])->where('katalog_id',$katalog_id)->where('sumber_dana_id',$sumber_dana_id)->whereIn('supplier_id',$supplier_ids)
