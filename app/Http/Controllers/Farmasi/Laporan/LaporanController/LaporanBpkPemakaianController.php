...
class LaporanBpkPemakaianController extends Controller
{
    public function get($request)
    {
        ...
        {
            $format_month = date('F',strtotime($current_date->copy()));
            $date[] =$format_month;
        ...
        foreach ($items as $item){
            if($last_item != $item->nama){
                ...
                foreach ($date as $dat){
                    $new_data[$item->nama][$dat] = $item->$dat;
                }
            }else{
                $new_data[$item->nama]['stok_awal'] += $item->stok_awal;
                foreach ($date as $dat){
    ...
    private function stokLog($farmasi_ids,$start_date,$end_date,$item_template_ids)
    {
        ...
        while($current_date <= $end_date)
        {
            $current_start  = $current_date->copy()->startOfMonth()->format('Y-m-d H:i:s');
            $current_end  = $current_date->copy()->endOfMonth()->format('Y-m-d H:i:s');
            $format_month = date('F',strtotime($current_date->copy()));
            $query_select .="$format_month,";
            $query_select2 .="
                IFNULL(laporan_$format_month.jumlah_min,0) AS $format_month,
            ";
            $query_format .= "
                    LEFT JOIN
                    (
                        ...
                        (
                            ...
                            AND ld.created_at >= '$current_start'
                            AND ld.created_at <= '$current_end'
                        ...
                        GROUP BY id
                    ) laporan_$format_month
                    ON items_table.item_id = laporan_$format_month.id";
            $current_date->addMonth();
        }
