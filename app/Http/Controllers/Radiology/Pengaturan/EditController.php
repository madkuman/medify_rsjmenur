...
class EditController extends Controller
{
    ...
    public function update(Request $req)
    {
    	try {
            ...
            return redirect('radiologi/pengaturan')->with('status', 'success')->with('message', 'Berhasil Mengubah Layanan');
            }
    	} catch (\Exception $e) {
            app('App\Http\Controllers\Error\Handler')->bugsnag($e);
            DB::connection('keuangan')->rollback();
    		return redirect('radiologi/pengaturan')->with('status', 'error')->with('message', 'Gagal Mengubah Layanan');
    	}
    }
}