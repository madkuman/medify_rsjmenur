...
class DaftarJabfungController extends Controller
{
	...
	{	
		...
		$data['pegawai'] = Pegawai::where('status_aktif','Aktif')
		->where(function($q){
			$q->where('pns_jabatan_fungsional','jabfung')->orWhere('pns_jabatan_fungsional','Jabfung');
		})
		...
		->orderBy('print_order','desc')->get();
		$data['ttd'] = TandaTangan::find($request->ttd_id);
		$pdf = MPDF::loadView('kepegawaian.laporan.hasil.daftar-jabfung.index',$data, [], [
			'format' => 'legal-L',
			'orientation' => 'L'
		]);
		$filename = 'Laporan-Daftar-Jabfung.pdf';
