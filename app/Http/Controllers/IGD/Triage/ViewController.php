...
class ViewController extends Controller
{
	public function index()
	{
        ...
        $date_filter = Carbon::now()->subDays(2)->endOfDay();

		$triage = Triage::with('creator')->where('created_at', '>', $date_filter)->orderBy('id','desc')->get();
		...
		// return view('igd.triage.index',$data);
		return view('igd.triage.baru', $data);
	}
    ...
    public function getTriage($kasus_id)
    {
	$triage=Triage::where('kasus_id',$kasus_id)->first();
	if(!empty($triage)){
	    $score=$triage->score;
        return $score;
    }
