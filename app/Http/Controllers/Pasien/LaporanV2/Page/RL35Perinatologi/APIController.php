...
namespace App\Http\Controllers\Pasien\LaporanV2\Page\RL35Perinatologi;
...
class APIController extends Controller
{
    ...
    {
		$total = MasterSIRSKegiatanPerinatologi::count('id');
		return json_encode([
			'status' => 200,
			'data' => $total
		]);
    ...
    public function getData(Request $request)
    {
		...
		$all_jenis_kegiatan = MasterSIRSKegiatanPerinatologi::skip($data_fetched)->take($limit)->orderBy('nomor')->get();
		$transaksi = PiutangDetail::select('piutang_detail.id as piutang_id','tarif_kategori.jenis_kegiatan_perinatologi as jenis_kegiatan_id')
