...
namespace App\Http\Controllers\Pasien\LaporanV2\Page\DataRincianRanap;
...
class APIController extends Controller
{
    ...
    public function getData(Request $request)
    {
		...
		$data = Kasus::where('tipe_ri',1)->whereBetween('created_at',[$start,$end])->with('pasien', 'pasien.alamat_kota.provinsi','pasien.alamat_kecamatan','pasien.alamat_kelurahan','pasien.agama', 'pasien.pendidikan','pasien.pernikahan', 'pasien.tni_keanggotaan', 'pasien.tni_kotama', 'pasien.tni_satker', 'pasien.tni_pangkat', 'pasien.wali',
			...
			'pembayaran.perusahaan', 'pembayaran.kelas', 'identitas', 'rawat_inap_transaksi_first.tempat_tidur.ruangan.lokasi',
		...
		foreach($data as $index => $item)
		{
			...
			$new_item->lokasi = $item->rawat_inap_transaksi_first->tempat_tidur->ruangan->lokasi->nama ?? '';
			...
			$new_item->tanggal_krs = $item->krs_at ? date("d-m-Y", strtotime($item->krs_at)); : '';
			$new_item->ruangan_keluar = $item->krs_at ? $item->lokasi->lokasi->nama : '';
			$new_item->status_keluar = $item->krs_at ? $item->krs_status : '';
			$new_item->alasan_keluar = $item->krs_at ? $item->krs_alasan : '';
