...
class APIController extends Controller
{
    public function getTotalData(Request $request)
    {
		...
		$total = Kasus::where('tipe_mc',1)->whereBetween('created_at',[$start,$end])->count('id');
    ...
    public function getData(Request $request)
    {
		...
		$data = Kasus::where('tipe_mc',1)->whereBetween('created_at',[$start,$end])->with('pasien', 'pasien.alamat_kota.provinsi','pasien.alamat_kecamatan','pasien.alamat_kelurahan','pasien.agama', 'pasien.pendidikan','pasien.pernikahan', 'pasien.tni_keanggotaan', 'pasien.tni_kotama', 'pasien.tni_satker', 'pasien.tni_pangkat', 'pasien.wali',
			...
			'pembayaran.perusahaan', 'pembayaran.kelas', 'identitas', 'urikkes_transaksi.transaksi_detail',
		...
		foreach($data as $index => $item)
		{
			...
			foreach($item->urikkes_transaksi->transaksi_detail as $trans_detail)
			{
				$nama_paket = $trans_detail->paket->nama ?? '';
				$array_paket[] = $nama_paket;
				$array_paket_unique = array_unique($array_paket);
			}

			$new_item->paket = implode(",", $array_paket_unique);
