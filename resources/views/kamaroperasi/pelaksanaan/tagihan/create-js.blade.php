...
<script type="text/javascript">
    ...
    {   
        ...
        if($('#tindakan-perawat').val() == '')
        {
            var search_tindakan_url = API_URL+'/kasus/datamedis/tindakan/get-top-tindakan-user?limit=50&kelas={{$trans_info->kasus->kelas->id}}&tipe='+tipe+'&persen=0'
        }
        else if(panjang.length >= 0)
        {
            var search_tindakan_url = API_URL+"/keuangan/tarif/search?keyword="+keyword+'&kelas={{$trans_info->kasus->kelas->id}}&tipe='+tipe+'&persen=0'
        }
        var flag = 1;
        var input;
        var harga;
    ...
    {   
        ...
        var tarif_kelas = "{{$trans_info->kasus->kelas->id}}";
    ...
    function tindakanEditModal(id)
    {
        $('#loading-top').fadeIn();
        $.ajax({
            url: API_URL + '/kasus/{{$trans_info->kasus->nomor_kasus}}/datamedis/tindakan/'+ id,
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                ...
                if ($.trim(icd_9).length == 0) {
                    $.ajax({
                        url: API_URL + '/kasus/{{$trans_info->kasus->nomor_kasus}}/tagihan/detail/'+ data.tagihan_detail_id,
                        type: 'GET',
                        dataType: 'json',
                        success: function(data) {
        ...
        var initAutoComplete = function(){
            jQuery('#modal-create-tindakan .tindakan-autocomplete').autoComplete({
                ...
                source: function(term, suggest){
                    ...
                    var search_tindakan_url = API_URL+"/keuangan/tarif/searchtarif?keyword="+term+'&kelas={{$trans_info->kasus->kelas->id}}&departemen={{$trans_info->kasus->lokasi->lokasi->departemen->id or "0"}}'
                    $.ajax({
                        url: search_tindakan_url,
                        type: 'GET',
                        dataType: 'json',
                ...
                onSelect: function(event, term, item) {
                    ...
                    //$("#tarif_kelas").val("{{$trans_info->kasus->kelas->kategori}}");
        ...
        var initAutoComplete = function(){
            jQuery('#modal-edit-tindakan .tindakan-autocomplete').autoComplete({
                ...
                onSelect: function(event, term, item) {
                    ...
                    $("#tarif_kelas_edit").val("{{$trans_info->kasus->kelas->kategori}}");
