...
<div class="row">
  <div class="col-12 form-group">
    ...
    <select class="js-select2 form-control" name="dokter" id="dokter_dropdown" data-placeholder="Pilih Dokter" required>
...
<div class="row">
  <div class="col-12 form-group">
    ...
    <input type="text" autocomplete="off" class="js-datepicker form-control tanggal_operasi" name="tanggal_operasi"
    {{-- data-date-start-date="{{date('d-m-Y')}}" 
    --}} id="tanggal_operasi" data-week-start="1" data-autoclose="true" data-today-highlight="true" data-date-format="dd/mm/yyyy" placeholder="Pilih Tanggal" value="{{ $permintaan->jadwal_operasi ? \Carbon\Carbon::parse($permintaan->jadwal_operasi)->format('d/m/Y') : '' }}" required>
  </div>
</div>
...
<div class="row">
  <div class="col-12 form-group">
    ...
    <select class="js-select2 form-control" name="ruangan" id="ruangan_dropdown" data-placeholder="Pilih Ruangan" required>
      <option></option>
      @foreach ($ruangans as $ruangan)
      <option value="{{ $ruangan->id }}" {{ $permintaan ? $ruangan->id == $permintaan->ruangan_id ? 'selected' : '' : '' }}>{{ $ruangan->name }}</option>
      @endforeach
    </select>
  </div>
</div>
<div class="row">
  <div class="col-12 form-group">
    ...
    <select class="js-select2 form-control" name="ronde" id="ronde_dropdown" data-placeholder="Tanggal dan Ruangan operasi harus terisi" disabled required>
      <option></option>
    </select>
  </div>
</div>
<div class="row">
  <div class="col-12 form-group">
    ...
    <input type="text" class="form-control" autocomplete="off" name="judul_operasi" id="judul_operasi" placeholder="Masukkan judul" value="{{ $permintaan->icd10->long_desc }}" required>
  </div>
</div>
  ...
  <div class="col-12 form-group">
    <div id="field-container">
      <div class="form-group row">
        <input type="text" class="col-md-10 form-control ml-15 child-field" autocomplete="off" name="judul_child[]" value="" placeholder="Masukkan Judul Operasi">
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          $("#ronde_dropdown").append('<option></option>');
					...
					if (initials.ronde && initials.tanggal == $("#tanggal_operasi").val() && initials.ruangan == $("#ruangan_dropdown").val()) {
          ...
          $.each(data, function(key, value){
              ...
              if (is_correct_ruangan_ronde && initials.ronde < value && not_yet_appended) {
               $("#ronde_dropdown").append('<option value="'+initials.ronde+'" selected>Ronde '+initials.ronde+'</option> ');
               not_yet_appended = false;
             }
             $("#ronde_dropdown").append('<option value="'+value+'">Ronde '+value+'</option> ');
           });
