                                    ...
                                    <form action="" method="post" id="pendaftaran_form">
                                        ...
                                        <div class="row">
                                            <div class="col-12 form-group">
                                                <label>Pasien</label>
                                                <select class="js-select2 form-control" name="pasien" id="pasien_dropdown"
                                                    data-placeholder="Pilih Pasien" required>
                                                    @if ($permintaan)
                                                        <option value="{{ $permintaan->pasien_detail->id }}">
                                                            {{ $permintaan->pasien_detail->name }}</option>
                                                    @endif
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-12 form-group">
                                                ...
                                                <select class="js-select2 form-control" name="kasus_id" id="kasus_dropdown"
                                                    data-placeholder="Pilih Kasus" required>
                                                </select>
                                            </div>
                                        ...
                                        <div class="block block-bordered" style="max-height: 300px">
                                            <div class="block-content px-5 py-5">
                                                ...
                                                <table class="table table-borderless" id="main_userdetail"
                                                    style="display: none;">
                                                    <tbody>
                                                        <tr>
                                                            <td>
                                                                <div class="row">
                                                                    <div class="col-3 text-center">
                                                                        <div class="vertical-align-center">
                                                                            <img class="img-avatar" id="thumb_pasien"
                                                                                src="{{ url('') }}/assets/img/placeholder.jpg"
                                                                                alt="">
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-9">
                                                                        ...
                                                                        <strong style="font-size: 13pt;"
                                                                            id="nama_pasien">Nama Pasien</strong> <br>
                                        ...
                                        <div class="row">
                                            <div class="col-12 form-group">
                                                ...
                                                <select class="js-select2 form-control" name="jenis_spesialis_id"
                                                    id="jenis_spesialis_dropdown" data-placeholder="Pilih Spesialis"
                                                    required>
                                                    <option value="" selected="" disabled="">Pilih</option>
                                                    @foreach ($spesialis_operasi as $item)
                                                        <option value="{{ $item->id }}"
                                                            @if ($permintaan) @if ($permintaan->jenis_spesialis_id == $item->id) selected @endif
                                            ...
                                            <div class="row">
                                                <div class="col-12 form-group">
                                                    ...
                                                    <input type="text" autocomplete="off"
                                                        ...
                                                        data-date-start-date="{{ date('d-m-Y') }}" name="masa_tunggu"
                                                        id="masa_tunggu" data-week-start="1" data-autoclose="true"
                                                        data-today-highlight="true" data-date-format="dd/mm/yyyy"
                                                        placeholder="Pilih Tanggal"
                                                        value="{{ $permintaan && $permintaan->masa_tunggu ? \Carbon\Carbon::parse($permintaan->masa_tunggu)->format('d/m/Y') : '' }}"
                                                        required>
                                                </div>
                                            </div>
                                        @endif
...
@section('js')
    <script>
        ...
        @if ($permintaan)
            var initials = {
                'pasien': {
                    id: {{ $permintaan->pasien_detail->id }},
                    text: "{{ $permintaan->pasien_detail->name }}"
                },
                @if ($permintaan->dokter)
                    'dokter': {
                        id: {{ $permintaan->dokter->id }},
                        text: "{{ $permintaan->dokter->name }}"
                    },
                @else
                    'dokter': null,
                @endif
                'diagnosis': {
                    id: "{{ $permintaan->diagnosis }}",
                    text: "{{ $permintaan->diagnosis }}"
                },
                'ronde': {{ $permintaan->nomor_ronde ? $permintaan->nomor_ronde : 'null' }},
                'ruangan': {{ $permintaan->ruangan_id ? $permintaan->ruangan_id : 'null' }},
                'tanggal': {!! $permintaan->jadwal_operasi
                    ? "'" . \Carbon\Carbon::parse($permintaan->jadwal_operasi)->format('d/m/Y') . "'"
                    : 'null' !!},
            };
        @else
            var initials = {
