    ...
    <div class="container">
        ...
        <div class="block">
            <div class="block-content pb-20 @if(empty($bed->transaksi->pasien)) bg-secondary-lighter @endif ">
                <div class="row p-0 m-0">
                    ...
                    <div class="col-md-3 h-100 d-flex align-self-center">
                        ...
                        <br>
                            <small class="font-w400">
                             ...
                             if(!empty($bed->transaksi)){
                                if(!empty($bed->transaksi->waktu_masuk)){
                                     $tgl_daftar = date('d M Y, H:i',strtotime($bed->transaksi->waktu_masuk));
                                }

                                if(!empty($bed->transaksi->kedatangan_at)){
                                    $tgl_datang = date('d M Y, H:i',strtotime($bed->transaksi->kedatangan_at));
                                }
                             }
                             @endphp
                             Daftar : {{$tgl_daftar}}
                             ...
                             Datang : {{$tgl_datang}}
                             <br>
                             <span class="dpjp"> DPJP : {{$bed->transaksi->kasus->admin->user->name ?? '-'}} </span>
                             @php
                            ...
                            Masa Rawat inap : 
                            @if ($d < 3)
                            <span class="badge badge-info">{{$d}} Hari</span>
                            @else
                            <span class="badge badge-danger">{{$d}} Hari</span>
                            @endif 
                            </small>
                         @endif
                            </h4>
                        ...
                        <div class="col-md-3 h-100 d-flex align-self-center mt-10">
                            @if(!empty($bed->transaksi))
                                @if(is_null($bed->transaksi->kedatangan_at))
                                    <form action="{{url('rawatinap/transaksi/konfirmasi')}}/{{$bed->transaksi['id']}}" method="POST" style="width: 100%;">
                                        {{csrf_field()}}
                                    <button class="btn btn-success btn-block btn-click-animate" type="submit">Konfirmasi</button>
                                    </form>
                                @elseif($bed->transaksi->kasus_id!=0)
                                    <a href="{{url('kasus')}}/{{$bed->transaksi->kasus->nomor_kasus}}" class="btn btn-primary btn-block">Kunjungi</a>
                                @endif
                            @endif
                        </div>
                    </div>
                ...
                @if(!empty($bed->transaksi))
                @include('rawatinap.bangsal.components-transaksi-file',['transaksi' => $bed->transaksi])
                @endif
                ...
                <div class="block-content pb-20 @if(empty($bed->transaksi->pasien)) bg-secondary-lighter @endif ">
                            ...
                            <br>
                                <small class="font-w400">
                                 ...
                                 No RM : <span class="no_rm"> {{$bed->booking->pasien->no_rm}}</span>
                                    ...
                                    @php
                                        ...
                                        if(!empty($bed->booking)){
                                           if(!empty($bed->booking->waktu_masuk)){
                                                $tgl_daftar = date('d M Y, H:i',strtotime($bed->booking->waktu_masuk));
                                           }

                                           if(!empty($bed->booking->kedatangan_at)){
                                               $tgl_datang = date('d M Y, H:i',strtotime($bed->booking->kedatangan_at));
                                           }
                                        }
                                    @endphp
                                    Daftar : {{$tgl_daftar}}
                                    ...
                                    Datang : {{$tgl_datang}}
                                    <br>
                                    <span class="dpjp"> DPJP : {{$bed->booking->kasus->admin->user->name ?? '-'}} </span>
                                </small>
                            @endif
                                </h4>
                            </div>
                            ...
                            <div class="col-md-2 h-100 d-flex align-self-center">
                                @if(!empty($bed->booking))
                                    @if(is_null($bed->booking->kedatangan_at))
                                        <form action="{{url('rawatinap/transaksi/konfirmasi')}}/{{$bed->booking['id']}}" method="POST" style="width: 100%;">
                                            {{csrf_field()}}
                                        <button class="btn btn-success btn-block btn-click-animate" type="submit">Konfirmasi</button>
                                        </form>
                                    @elseif($bed->booking->kasus_id!=0)
                                        <a href="{{url('kasus')}}/{{$bed->booking->kasus->nomor_kasus}}" class="btn btn-primary btn-block">Kunjungi</a>
                                    @endif
                    ...
                    @if(!empty($bed->booking))
                    @include('rawatinap.bangsal.components-transaksi-file',['transaksi' => $bed->booking])
                    @endif
                    @endif
                </div>
            </li>
