	...
	<div class="block p-10">
		<div class="block-header">
			<h3 class="block-title">
				Rekap Diet Pesanan @if(!empty($date))<small>{{$date}}</small>@else<small>{{date('d F Y')}}</small>@endif
			</h3>
		</div>
		@include('gizi.pemesanan.components.filter')
		<div class="block-header">
			<h3 class="block-tittle">
				<small><a href="{{url('gizi/pemesanan/print/rekap-jp')}}?tanggal=
				@if(!empty($date))
				{{$date}}
				@else
				{{date('d F Y')}}
			...
			<table class="table-responsive table table-bordered  table-hover table-striped table-center">
				<tbody>
					<tr>
						...
						@foreach($jenis as $jp)
						<th class="text-center" colspan="6">{{$jp->jenis}}</th>
						@endforeach
