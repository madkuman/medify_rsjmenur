			...
			<table class="table-responsive table table-bordered  table-hover table-striped table-center">
				<tbody>
					...
					<tr class="border-top-bold">
						...
						@for($i=1;$i<=3;$i++)
						@foreach($kelas as $kelas_item)
						<td>{{$data[$kombinasi[$y]][$i][$kelas_item->nama]['normal']}}</td>
						@endforeach
						<td>{{$jumlah[$kombinasi[$y]][$i]['normal']}}</td>
						@endfor
						<td>{{$jumlah[$kombinasi[$y]]['normal']}}</td>	
					</tr>
					<tr class="border-bottom-bold">
						<td><i>Tambahan {{$kombinasi[$y]}}</i></td>
						@for($i=1;$i<=3;$i++)
						@foreach($kelas as $kelas_item)
						<td>{{$data[$kombinasi[$y]][$i][$kelas_item->nama]['tambahan']}}</td>
						@endforeach
						<td>{{$jumlah[$kombinasi[$y]][$i]['tambahan']}}</td>
						@endfor
						<td>{{$jumlah[$kombinasi[$y]]['tambahan']}}</td>	
					</tr>
					@php $y++; @endphp
					@endforeach
				</tr>
