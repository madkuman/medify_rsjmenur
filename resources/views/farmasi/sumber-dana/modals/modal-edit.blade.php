<div class="modal" id="modal-normal" role="dialog" aria-labelledby="modal-normal" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form method="POST" enctype="multipart/form-data" action="{{url('farmasi/'.session('farmasi')->slug.'/sumber-dana')}}/edit">
            {{csrf_field()}}
            <input type="hidden" name="id" value="{{$sumber_dana->id}}">
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
                    <div class="block-header">
                        <h3 class="block-title">Ubah Sumber Dana</h3>
                    </div>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="nama" placeholder="Nama Sumber Dana" value="{{$sumber_dana->nama}}" required>
                                </div>
                            </div>
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <select class="js-select2 form-control" name="kategori_id" style="width: 100%;" data-placeholder="Pilih Kategori">
                                        @foreach($kategori as $supp)
                                            <option value="{{$supp->id}}" @if($supp->id == $sumber_dana->kategori_id) selected @endif>{{$supp->nama}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
