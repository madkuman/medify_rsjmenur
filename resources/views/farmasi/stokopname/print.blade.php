    ...
    <table class="bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$row['nama']}}</td>
                <td>{{$row['satuan']}}</td>
                <td class="righted">Rp {{number_format($row['harga'])}}</td>
                <td>{{$row['sebenarnya']}}</td>
                <td>{{date('d F Y', strtotime($row['kadaluarsa']))}}</td>
                <td>{{$row['keterangan']}}</td>
                <td class="righted">Rp {{number_format($row['sebenarnya'] * $row['harga'])}}</td>
            ...
            <tr>
                ...
                <th class="righted" style="padding-left: 0px;">Rp {{number_format($total)}}</td>
            </tr>
        </tbody>
    </table>
    <br>
