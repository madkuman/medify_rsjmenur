    <script type="text/javascript">
        ...
        $('#btn-add').on('click', function(){
            ...
            if($('#jumlahDiambil').val() == '') 
            {
                $('#jumlahDiambil').parent('div').find('.text-warning').html('Harap diisi !');
                flag++;
            }
            else $('#jumlahDiambil').parent('div').find('.text-warning').html('');
            var namaObat;
            var namaObatVal;
            var input = {};
            if(harian)
            ...
            if (jenisObat == 'generik') {
                ...
                input.jumlah_awal = nomorObat;
                input.jumlah = $('#jumlahDiambil').val();
        ...
        function reseFields() {
            ...
            $('#jumlahDiambil').val(null).trigger('change');
        ...
        function changeResepJadi() {
            $('.resep-jadi').on('click', function(){
                ...
                else $('#jumlahObat').val(input.jumlah_awal);
                $('#jumlahDiambil').val(input.jumlah);
        ...
        $('#btn-save').on('click', function(){
            ...
            if (jenisObat == 'generik') {
                ...
                resepJadi += 
                    ...
                    <div class="font-size-sm text-muted">`+ tipeObat +`</div>
                    <div class="font-size-sm text-muted">`+ namaObat +`</div>
                    <div class="font-size-sm text-muted">Jumlah Awal: `+ nomorObat; 
                if(harian) resepJadi += ` (7 Hari : `+hari7+`, 23 Hari : `+hari23+`, Duk RS : `+dukRS+`)`;
                resepJadi+=` - Jumlah Diambil: ` + input.jumlah;
                resepJadi += `<div class="font-size-sm text-muted"> Aturan : `+ aturanPenggunaan +`</div></div>`;
                if(typeof keteranganObat != 'undefined')
                    resepJadi += `<div class="font-size-sm text-muted">Keterangan : `+ keteranganObat
                    resepJadi += `<input type="hidden" name="input[]" class="resep-input" value='`+ JSON.stringify(input) +`'>`;
                    /*<input type="hidden" name="jenisObatVal" value="`+ jenisObat +`">
