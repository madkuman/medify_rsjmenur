...
<body>
   ...
   <table width="100%" style="line-height: 6px;">
      <tr>
         ...
         <td width="35%" style="font-size: 6pt; padding-left: 3.5px">Tgl Lhr: @if (!empty($transaksi->pasien_detail->date_of_birth)) {{ date('d/m/Y', strtotime($transaksi->pasien_detail->date_of_birth)) }} @else - @endif</td>
      </tr>
      <tr>
         ...
         <td style="font-size: 6pt; padding-left: 3.5px">No. RM <span style="font-size: 8pt">{{ $transaksi->pasien_detail->no_rm ?? '-' }}</span></td>
      </tr>
      <tr>
         ...
         <td>
            <table width="100%" style="margin: 0">
               <tr>
                  <td style="font-size: 6pt">Jumlah <span style="font-size: 8pt">{{ $detail->jumlah }} {{ $detail->satuan }}</span></td>
               </tr>
               <tr>
                  <td style="font-size: 6pt">Exp. @if(!empty($exp_dates[$i])) {{ $exp_dates[$i] }} @else - @endif</td>
               </tr>
            </table>
         </td>
      </tr>
      <tr>
         <td colspan="3" style="font-size: 6pt; word-wrap: break-word;">Aturan Pakai : <span style="font-size: 8pt; line-height: 8px">{{ $detail->aturan ?? '-' }}</span></td>
      </tr>
      <tr>
