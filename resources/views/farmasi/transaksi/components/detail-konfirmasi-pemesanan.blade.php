        ...
        <table class="table table-vcenter">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <input type="number" class="form-control input-embalase d-none" id="embalase-{{$i}}" name="embalase[]" value="{{ $detail->embalase }}" placeholder="Embalase" onchange="changeTotal()">
                        <a href="javascript:void(0)" class="no-border editable editable-click">{{ $detail->embalase }}</a>
                    </td>
                ...
                <tr class="table-success">
                    ...
                    <td class="text-right" id="subtotal-harga">Rp. {{number_format($transaksi->total_biaya_obat)}}</td>
                </tr>
                <tr>
                    {{-- @if($transaksi->status == 1) <td></td> @endif --}}
                ...
                <tr>
                    ...
                    <td class="text-right" id="subtotal-obat-harga">Rp. {{number_format($transaksi->total_biaya_obat - $transaksi->embalase + $transaksi->total_retur)}}</td>
                </tr>
                @if($transaksi->pembayaran_detail && $transaksi->pembayaran_detail->perusahaan->tipe->slug == 'bpjs' && $farmasi->perharian)
                <tr>
                    {{-- @if($transaksi->status == 1) <td></td> @endif --}}
    ...
    <div class="mt-30 ">
        <div class="form-group row">
            <div class="col-12">
                ...
                @if ($transaksi->status_kasir != 1)
                    <button type="submit" class="btn btn-warning" form="form-batal-konfirmasi-permintaan">Batal Konfirmasi Permintaan</button>
                @endif
            </div>
            @if($transaksi->status_kasir == 1)
                @if($status_bayar == 0)
...
@if(!empty($transaksi->dikerjakan_at))
    <div style="position: relative;">
        <img src="{{asset('assets/img/paid_stamp.png')}}" width="130" style="position: absolute; top: -90px; left: 20px;z-index:1004">
    </div>
@endif
@if (count($transaksi->copy_resep) == 0 && $transaksi->retur->count() == 0)
    @if ($transaksi->status_kasir != 1)
        <br>
        <button type="submit" class="btn btn-warning mt-3" form="form-batal-konfirmasi-pemesanan">Batal Konfirmasi Pemesanan</button>
    @endif
@endif
<form method="POST" action="{{ url('farmasi/' . session('farmasi')->slug . '/transaksi/batal-konfirmasi-permintaan') }}" id="form-batal-konfirmasi-permintaan">
    {{ csrf_field() }}
    <input type="hidden" name="id" value="{{ $transaksi->id }}">
</form>
<form method="POST" action="{{ url('farmasi/' . session('farmasi')->slug . '/transaksi/batal-konfirmasi-pemesanan') }}" id="form-batal-konfirmasi-pemesanan">
    {{ csrf_field() }}
    <input type="hidden" name="id" value="{{ $transaksi->id }}">
</form>