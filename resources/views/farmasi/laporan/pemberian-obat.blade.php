...
<head>
    ...
    <title>Laporan Pemberian Obat</title>
    <style>
    body { 
        font-family: Calibri; 
        color: black 
...
<body>
    ...
    <div class="mt-25">
        <div class="text-center header-title">
            <b>LAPORAN PEMBERIAN OBAT <br> TANGGAL {{date('d F Y',strtotime($min_date))}} - {{date('d F Y',strtotime($max_date))}}</b>
        </div>
    </div>
    <div class="mt-10">
        <table class="text-small">
            <tr>
                <td>No. RM</td>
                <td>:</td>
                <td>{{$pasien ? $pasien->no_rm_formatted : "-"}}</td>
            </tr>
            <tr>
                <td>Nama Pasien</td>
                <td>:</td>
        ...
        <table class="table table-bordered text-small">
            <thead>
                <tr>
                    ...
                    <th align="center" width="80px">KODE OBAT</th>
                    <th align="center">NAMA OBAT</th>
                    <th align="center" width="100px">ATURAN</th>
                    <th align="center" width="80px">SATUAN</th>
                    <th align="center" width="80px">JUMLAH</th>
                    <th align="center" width="100px">HARGA SATUAN</th>
                    <th align="center" width="75px">HARGA</th>
                </tr>
            </thead>
            <tbody>
                @php $i=1 @endphp
                @php $tanggal_loop = 'init' @endphp
                <?php $total_1=0; $total_2=0; $total_3=0;?>
                <?php $total_pertanggal_1=0; $total_pertanggal_2=0; $total_pertanggal_3=0;?>
                @foreach($items as $key => $row)
                @if($tanggal_loop != date('d-M-Y',strtotime($row->paid_at)) || $tanggal_loop == 'init')
                <tr>
                    <td align="center" colspan="2" class="br-white">
                        <b>No. Antrian : {{$row->nomor_antrian ? $row->final_detail->nomor_antrian : "-"}}</b>
                    </td>
                ...
                @endif
                @php $tanggal_loop = date('d-M-Y',strtotime($row->paid_at)) @endphp
                ...
                <tr>
                    ...
                    <?php $total_1+=$total ?>
                    <?php $total_pertanggal_1+=$total ?>
                    <td align="right">{{number_format($res->harga)}}</td>
                    <?php $total_2+=$res->harga ?>
                    <?php $total_pertanggal_2+=$res->harga ?>
                    <td align="right">{{number_format($res->harga * $total)}}</td>
                    <?php $total_3+=$total * $res->harga ?>
                    <?php $total_pertanggal_3+=$total * $res->harga ?>
                </tr>
                @endforeach
                @if(!isset($items[$key+1]->paid_at) || $tanggal_loop != date('d-M-Y',strtotime($items[$key+1]->paid_at)))
                <tr>
                    <td colspan="5" class="text-center">
                        Total {{$tanggal_loop}}
                    </td>
                    <td align="right">{{number_format($total_pertanggal_1)}}</td>
                    <td align="right">{{number_format($total_pertanggal_2)}}</td>
                    <td align="right">{{number_format($total_pertanggal_3)}}</td>
                </tr>
                @php $total_pertanggal_1 = 0 @endphp
                @php $total_pertanggal_2 = 0 @endphp
                @php $total_pertanggal_3 = 0 @endphp
                @endif
                @endforeach
                <tr>
                    <td align="center" colspan="5" style="font-weight: bold;">T O T A L</td>
