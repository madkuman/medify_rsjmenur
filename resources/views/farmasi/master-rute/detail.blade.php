...
@section('content')
	<div class="block">
        <div class="block-header bordered">
            <h3 class="block-title">
            	<small>Master Rute</small> <br>
            	{{$rute->nama}}
            </h3>
            <div class="block-options">
                <form method="POST" action="{{url('farmasi/'.session('farmasi')->slug.'/master-rute/delete')}}">
                    {{csrf_field()}}
	            ...
	            <button type="button" class="btn btn-secondary btn-square" id="edit-rute">
            ...
            <div class="block block-transparent">
                <div class="row">
                	<div class="col">
                		...
                		<h5>{{$rute->creator->name}}</h5>
                	</div>
                	<div class="col">
                        <label>TANGGAL DIBUAT</label>
                        <h5>{{ date('d F Y', strtotime($rute->created_at)) }}</h5>
                	</div>
                </div>
            </div>
        </div>
    ...
    @include('farmasi.master-rute.modals.modal-edit')
@endsection
