	...
	<div class="block">
        <div class="block-header bordered">
            <h3 class="block-title">
            	<small>Katalog</small> <br>
            	{{$katalog->nama}}
            </h3>
            <div class="block-options">
                <form method="POST" action="{{url('farmasi/'.session('farmasi')->slug.'/katalog/delete')}}">
                    {{csrf_field()}}
	            ...
	            <button type="button" class="btn btn-secondary btn-square" id="edit-katalog">
            ...
            <div class="block block-transparent">
                <div class="row">
                	<div class="col">
                		...
                		<h5>{{$katalog->creator->name}}</h5>
                	</div>
                	<div class="col">
                        <label>TANGGAL DIBUAT</label>
                        <h5>{{ date('d F Y', strtotime($katalog->created_at)) }}</h5>
                	</div>
                </div>
            </div>
        </div>
...
@section('js')
    <script type="text/javascript">
        $('#edit-katalog').on('click', function(){
