<script type="text/javascript">
    ...
    function deleteSep(){
        ...
        $.ajax({
            type: "POST",
            url: BASE_URL + "vclaim-v2/sep/"+objSep.no_sep+"/delete",
            cache: false,
            contentType: false,
            processData: false,
            headers: {
            ...
            success: function (res) {
                if(res.bpjs_enable == false) {
                    callSwal('warning', res.result, 0, "bpjs/sep/search");
                    return
                }
                result = JSON.parse(res.result);
                $('#delete').attr("disabled", false);
                if(result.metaData.code != 200){
                    swal('Gagal', result.metaData.message, 'warning')
                    .then(() => {
                        window.location.href = BASE_URL + 'bpjs/sep/search';
                    })
                }else{
            ...
            error: function (error) {
                ...
                $('#loading').hide();
                console.log(error)
                swal('Gagal', 'Gagal menghapus SEP, silahkan coba lagi', 'warning')
                .then(() => {
                    location.reload();
                })
            }
    ...
    $('#submit').on('click', function(e) {
        ...
        var jenisPelayanan = '{{$sep->jenis_pelayanan}}';
        if(jenisPelayanan == 1){
        ...
        formData.append('bpjs_jenis_pelayanan', '{{$sep->jenis_pelayanan}}');
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if(result.metaData.code != 200){
                    ...
                    swal('Sukses', 'SEP berhasil disimpan', 'success')
                    .then(() => {
                        window.location = `${BASE_URL}bpjs/sep/search?no_sep=${objSep.no_sep}`;
                    })
    ...
    function showPasien(obj) {
        ...
        var noBPJSbySEP = $('#no_bpjs_by_sep').val();
        ...
        for (var i = 0; i < pasien.pembayaran.length; i++) {
            ...
            if(pasienBPJS.perusahaan.type == 1) {
                // jika di db table BPJS_SEP, no_bpjsnya tidak null
                // dan sama dengan no asuransi pembayaran bpjs pasien
                if(noBPJSbySEP != null && noBPJSbySEP == pasienBPJS.no_asuransi) {
                    break;
                } else {
                    pasienBPJS = null;
                }
        ...
        // jika pasien pembayaran bpjs tidak sama dengan nomor bpjs di sep
        if (pasienBPJS == null) {
            swal('Gagal', 'No BPJS di SEP tidak sama dengan No BPJS pembayaran pasien', 'warning')
            .then(() => {
                location.reload();
            })
        }
        ...
        $('#preview_pasien_kelas_bpjs').text(pasienBPJS.kelas_id);
        $('#infoPasien').show();
    };
