        ...
        <div class="container">
            <div class="block rounded">
                ...
                <div class="block-content">
                    <form action="{{ url('bpjs/rujuk-balik/edit') }}" method="post" id="form-create">
                        <div class="row">
                            <div class="col-12 col-sm-12 col-md-8 col-lg-6">
                                <input type="hidden" name="id" value="{{ $rujuk_balik->id }}">
                                <div class="form-group">
                                    <label class="control-label">No SRB</label>
                                    <input type="text" name="no_sep" class="form-control" value="{{ $rujuk_balik->no_surat_rujuk_balik }}" readonly>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">No. SEP</label>
                                    <input type="text" name="no_sep" class="form-control" value="{{ $rujuk_balik->no_sep }}" readonly>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="kode_program_prb" class="form-control select-program-prb" data-placeholder="Pilih Program"
                                        style="width: 100%">
                                        <option value="{{ $rujuk_balik->kode_program_prb }}">{{ $rujuk_balik->program_prb }}</option>
                                    </select>
                                    <small class="text-danger error-message"></small>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Keterangan</label>
                                    <textarea name="keterangan" class="form-control" cols="30" rows="3">{{ $rujuk_balik->keterangan }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Saran</label>
                                    <textarea name="saran" class="form-control" cols="30" rows="3">{{ $rujuk_balik->saran }}</textarea>
                                </div>
                            </div>
                            <div class="col-12 col-sm-12 col-md-8 col-lg-6">
                                <h6>Tambah Obat</h6>
...
@section('js')
    <script type="text/javascript">
        ...
        $('.select-pasien').select2({
            ajax: {
                url: function(params) {
                    return API_URL + '/pasien/get-bpjs-non-krs/' + params.term;
                },
                dataType: 'json',
                beforeSend() {
        ...
        //Section Addition
        var obat_data = {!! json_encode($rujuk_balik->detail) !!};
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        ...
                        if (response.metaData.code == 200) {
                            callSwal('success','Berhasil',"Berhasil Mengubah Rujuk Balik","bpjs/rujuk-balik")
                        } else {
