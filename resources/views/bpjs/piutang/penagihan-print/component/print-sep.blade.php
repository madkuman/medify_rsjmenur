		...
		<div style="position: absolute; top: 0;">
			<table style="width: 100%">
				<tr>
					...
					<td style="width: 79%;">{{$bpjs_real[$key]->noSep ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Tgl.SEP</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->tglSep ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">No.Kartu</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->peserta->noKartu ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Nama Peserta</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->peserta->nama ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Tgl Lahir</td>
					<td class="text-light">:</td>
					@php 
					if(isset($bpjs_real[$key]))
					$tgl_lahir = implode("-", array_reverse(explode("-", $bpjs_real[$key]->peserta->tglLahir)));
					@endphp
					<td>{{$tgl_lahir ?? '-'}}</td>
				</tr>
				<tr>
					...
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->peserta->kelamin ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Poli Tujuan</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->poli ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Diagnosa Awal</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->diagnosa ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Catatan</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->catatan ?? '-'}}</td>
				</tr>
				<tr>
		...
		<div style="position: absolute; top: 51; left: 290;">
			<table style="width: 100vw">
				<tr>
					...
					<td style="width: 31%">{{$bpjs_real[$key]->peserta->jnsPeserta ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light" style="width: 25%">Jenis Rawat</td>
					<td style="width: 4%" class="text-light">:</td>
					<td style="width: 31%">{{$bpjs_real[$key]->jnsPelayanan ?? '-'}}</td>
				</tr>
				<tr>
					<td class="text-light">Kelas Rawat</td>
					<td class="text-light">:</td>
					<td>{{$bpjs_real[$key]->kelasRawat ?? '-'}}</td>
				</tr>
			</table>
		</div>
	</div>
