<div class="row mb-20">
    ...
    <div class="col-3">
        ...
        <select class="form-control split-pasien-pembayaran" id="pasien-pembayaran" name="pasien_pembayaran_id[]" style="width: 100%;" data-placeholder="Pilih Jenis Pembayaran">
            ...
            @foreach($pembayaran as $item)
                <option value="{{$item->id}}" data-perusahaan-id="{{$item->perusahaan->perusahaan_keuangan_id}}" @if($piutang->pasien_pembayaran_id == $item->id) selected @endif>{{$item->perusahaan->nama}}</option>
            @endforeach
        </select>
    </div>
    @endif
    <div class="col-3">
        ...
        <select class="js-select2 form-control split-perusahaan" data-id="1" name="perusahaan[]" style="width: 100%">
            ...
            <option value="{{$item->id}}" @if($piutang->perusahaan_id == $item->id) selected @endif>{{$item->nama}}</option>
            @endforeach
        </select>
    </div>
    <div class="col-2">
        <label>PJ Pembayaran</label>
        <input type="text" class="form-control split-pihak-ketiga" name="pihak_ketiga[]" value="{{$piutang->pihak_ketiga}}">
    </div>
    <div class="col-2">
        <label>Total</label>
        <input type="text" class="form-control split-total" name="total[]" value="{{$piutang->total}}">
