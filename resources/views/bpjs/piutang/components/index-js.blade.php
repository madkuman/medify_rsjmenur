...
$('#file_dropdown').on('change', function (e) {
    status_file = this.value;
    table.destroy();
    draw(filter,null,null);
});
...
function draw(filter,tanggal_start,tanggal_end){
    ...
    ajax: {
        ...
        url: API_URL + '/bpjs/penagihan/getbyfilter',
    },
    columns: [
        { data: 'external_sep_no', name: 'external_sep_no', className: 'text-center' },
        { data: 'judul', name: 'judul' },
        { data: 'perusahaan', name: 'perusahaan' },
        { data: 'krs_at', name: 'krs_at', className: "text-center",
            render: function ( data ) {
                if(data == '')
                    return '-';
                else
                  return (new Date(data)).toShortFormat();
            } 
        },
        ...
        { data: 'bayar',className: 'text-center', 
            render: function(data, type, row, meta){
                ...
                var file = res[2];
                if(file == 1)
                    var file_status = `<span class="badge badge-pill badge-success"><i class="fa fa-check mr-5"></i></span>`;
                else
                    var file_status = `<span class="badge badge-pill badge-danger"><i class="fa fa-times mr-5"></i></span>`;

                if (status == 'unpaid')
                    data = '<label class="css-control css-control-success css-checkbox">&nbsp;<input type="checkbox" data-pk="'+id+'" class="css-control-input centang" id="centang'+id+'">&nbsp;<span class="css-control-indicator"></span></label>'+file_status;
                else
...
$('#buttonCheckPage').on('click', function(){
    $.each($('.centang'), function(){
...
var transaksiModel, transaksiId, transaksiIdSerial;
...
$('#submit_penagihan').click(function() {
    var akun = $("#akun").val();
    window.location.href = `${BASE_URL}keuangan/piutang/rekap-penagihan?id=${transaksiIdSerial}&origin=bpjs&akun=${akun}&back_url=bpjs`;
})
