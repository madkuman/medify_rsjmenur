@extends('keuangan.layouts.main')

@section('title')
Daftar PJK - Keuangan
@endsection

@section('css')

<style>
.dataTables_processing {
    background-color: white;
}
</style>
@endsection
@section('content')
@include('keuangan.pjk.components.header')

<div class="row gutters-tiny">
    <div class="col-xl-4">
        <div class="block block-link-shadow text-right">
            <div class="block-content block-content-full clearfix">
                <div class="float-left mt-10">
                    <i class="si si-bag fa-3x text-body-bg-dark"></i>
                </div>
                <div class="font-size-h3 font-w600">{{$utang_num}}</div>
                <div class="font-size-sm font-w600 text-uppercase text-muted">Transaksi</div>
            </div>
        </div>
    </div>
    <div class="col-xl-4">
        <div class="block block-link-shadow text-right">
            <div class="block-content block-content-full clearfix">
                <div class="float-left mt-10">
                    <i class="si si-wallet fa-3x text-body-bg-dark"></i>
                </div>
                <div class="font-size-h3 font-w600">Rp {{number_format($utang_total,0)}}</div>
                <div class="font-size-sm font-w600 text-uppercase text-muted">Total PJK</div>
            </div>
        </div>
    </div>
    <div class="col-xl-4">
        <div class="block block-link-shadow text-right">
            <div class="block-content block-content-full clearfix">
                <div class="float-left mt-10">
                    <i class="si si-wallet fa-3x text-body-bg-dark"></i>
                </div>
                <div class="font-size-h3 font-w600">% {{number_format(($utang_total/$utang_rata*100)-100,2)}}</div>
                <div class="font-size-sm font-w600 text-uppercase text-muted">Naik dibanding rata rata</div>
            </div>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-md-12">
        <div class="block">
            <div class="block-header block-header-default py-20">
                <span><h4 class="mb-0">Transaksi PJK Belum Diproses</h4><hr>
                </span>
                <div class="block-options">
                    <a href="{{url()->current()}}/baru" class="btn btn-sm btn-primary btn-hero">
                        <i class="fa fa-plus"></i> Buat PJK
                    </a>
                </div>
            </div>
            <div class="block-content py-20">
                <table class="table table-striped table-hover table-vcenter js-dataTable-simple" id="transaksiTable">
                    <thead>
                        <tr>
                            <th class="text-center" style="width: 100px;">No. PJK  </th>
                            <th class="text-center" style="width: 300px;">Mengenai  </th>
                            <th class="text-center" style="width: 200px;">Tanggal  </th>
                            <th class="text-center" style="width: 250px;">Rekanan  </th>
                            <th class="text-right" style="width: 200px;">Total  </th>
                            <th class="text-right" style="width: 250px;">Aksi </th>
                        </tr>
                    </thead>
                </table>
            </div>
        </div>
    </div>
</div>
@endsection

@section('js')
@include('keuangan.pjk.components.index-js')
{{-- <script src="{{asset('js/keuangan/utang/index3.js')}}"></script> --}}


@endsection