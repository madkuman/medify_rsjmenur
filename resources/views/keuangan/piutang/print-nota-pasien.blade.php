...
<html>
<head>
    <title>Perincian Biaya Piutang - Pasien</title>
    <style type="text/css">
        @page{
            margin: 10mm;
...
<body>
    ...
    <table class="list">
        ...
        @if(!empty($piutang_details['Tindakan']))
        @php $temp_data['item_pertanggal'] = $piutang_details['Tindakan'] @endphp
        @php $temp_data['kategori'] = 'Tindakan' @endphp
        @include('keuangan.piutang.components-nota.pasien-content',$temp_data)
        @endif

        @if(!empty($piutang_details['Penunjang']))
        @php $temp_data['item_pertanggal'] = $piutang_details['Penunjang'] @endphp
        @php $temp_data['kategori'] = 'Penunjang' @endphp
        @include('keuangan.piutang.components-nota.pasien-content',$temp_data)
        @endif

        @if(!empty($piutang_details['Farmasi']))
        @php $temp_data['item_pertanggal'] = $piutang_details['Farmasi'] @endphp
        @php $temp_data['kategori'] = 'Farmasi' @endphp
        @include('keuangan.piutang.components-nota.pasien-content',$temp_data)
        @endif

        @if(!empty($piutang_details['Lain lain']))
        @php $temp_data['item_pertanggal'] = $piutang_details['Lain lain'] @endphp
