@foreach($operasi as $transaksi)
    ...
    <table class="borderless">
        ...
        <tr>
            ...
            @if(!empty($transaksi->parent))
            <td>{{$transaksi->parent->pasien_detail->id}}</td>
            @else
            <td>{{$transaksi->pasien_detail->id}}</td>
            @endif
        </tr>
    </table>
    <div class="box" style="margin-top: 20px">
        <table class="borderless center nopadding" style="width: 100%">
            <tr>
                @foreach ($transaksi->peran as $item)
                    <th>
                     {{$item->nama}}
                    </th>
                  @endforeach
            ...
            <tr>
            @foreach ($transaksi->peran as $peran_item)
                <td>
        ...
        <table class="borderless" style="width: 100%">
            ...
            <tr>
                <td style="vertical-align: top;">Material Digunakan</td>
                <td style="vertical-align: top;">:</td>
                <td>
                    @foreach($transaksi->pemakaian as $pemakaian)
                    @if($pemakaian->jenis == 'alkes') @continue @endif
                    {{$pemakaian->jumlah}} {{$pemakaian->getItem->nama}}<br>
                    @endforeach
                </td>
            </tr>
            <tr><td colspan="2">&nbsp;</td></tr>
