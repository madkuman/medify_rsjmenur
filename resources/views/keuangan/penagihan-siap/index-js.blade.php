...
function draw(filter,tanggal_start,tanggal_end){
    ...
    ajax: {
        ...
        url: API_URL + '/keuangan/penagihan-siap/getbyfilter',
    },
    columns: [
        ...
        { data: 'slug', name: 'id', className: 'text-center', 
            render: function(data, type, row, meta){
                data = `
                        <a href="penagihan-siap/${data}" class="btn btn-sm btn-alt-primary" data-toggle="tooltip" title="Lihat Detail Transaksi">&nbsp;<i class="fa fa-search-plus"></i></a>&nbsp
                        <a href="javascript:void(0);" class="btn btn-sm btn-alt-danger ml-5 btn-delete-penagihan-siap" data-id="${row?.id}" data-toggle="tooltip" title="Delete Transaksi">&nbsp;<i class="fa fa-trash"></i></a>&nbsp
                    `;
...
Date.prototype.toShortFormat = function() {
  ...
  $(document).on('click', '.btn-delete-penagihan-siap', function(){
    let id = $(this).data('id');
    ...
    swal({
            ...
            if(result?.value){
                ...
                $.ajax({
                    type: "POST",
                    url: `${BASE_URL}keuangan/penagihan-siap/delete`,
                    data: {
                        id
                    },
                    dataType: "json",
                    success: function (response) {
                        if(response?.status){
                            ...
                            return callSwal('success',title = response?.title,text = response?.message,url = 0)
                        }
                        else{
                            return callSwal('error',title = response?.title,text = response?.message,url = 0)
                        }
                    },
