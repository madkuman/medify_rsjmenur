<head>
	<title>Hasil Uji dan Pemeriksaan Riwayat Kesehatan</title>
	<style type="text/css">
		body{
...
<body>
	...
	<table style="width: 100vw; page-break-inside:avoid; border-collapse: collapse;" autosize="1">
		...
		<tr>
			...
			<td style="width: 2%; border: 1px solid white">:</td>
			<td style="width: 33%; border: 1px solid white">{{$kesatuan or "(-)"}} / {{$satker or "(-)"}}</td>
			@endif
			<td style="width: 17%; border: 1px solid white"></td>
		</tr>	
	</table>
	...
	<table style="width: 100vw; border-collapse: collapse;" autosize="1">
		...
		@if($type == "satker-pilihan" && $last_satker != $item->pasien_detail->tni_satker->nama)
		<tr>
			<td colspan="18" style="font-size: 12px; border: 1px solid black; font-weight: bold">
				{{$item->pasien_detail->tni_satker->nama}}
			</td>
		</tr>
		@endif
		@php($last_satker = $item->pasien_detail->tni_satker->nama)
		<tr>
			...
			<td style="padding-left: 5px; padding-right: 5px; text-align: center; font-size: 12px; border: 1px solid black; width: 8%">{{$item->pasien_detail->tni_pangkat->nama}}<br>{{$item->pasien_detail->tni_nrp}}</td>
			<td style="padding-left: 5px; padding-right: 5px; font-size: 12px; border: 1px solid black; width: 8%">{{$item->pasien_detail->tni_keanggotaan->nama}}</td>
