<script type="text/javascript">
    ...
    var table = $('#tableCuti').DataTable({
        ...
        { data: null, name: 'tanggal', render: function (data, type, row, meta) {
                content = '';
        ...
        { data: null, name: 'cuti_pengajuan_id', render: function (data, type, row, meta) {
                content = '';
                if(data.cuti_pengajuan_id != ''){
                    content =`<a class="btn btn-secondary" target="_blank" href="{{url('')}}/kepegawaian/cuti/pengajuan/form/`+data.cuti_pengajuan_id+`">Lihat</a></div>`;
                }
                return content;
            },
        },
        ...
        { data: null, name: 'id', render: function (data, type, row, meta) {
                ...
                content = '';
                if(data.cuti_pengajuan_id == ''){
                content =`<a class="btn btn-secondary btn-edit" href="javascript:void(0)" data-id="`+data.id+`" data-jenis-cuti="`+data.master_cuti_id+`" data-jumlah-cuti="`+data.kuota_perubahan+`" data-tanggal="`+data.tanggal_dmy+`">Edit</a></div>`;
                }
                return content;
            },
        },
