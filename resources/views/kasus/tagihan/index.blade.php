            ...
            <div class="col-lg-9 col-xl-9">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        @endif
                        @if($my_role == 1) 
                        @if(session('my_role_'.$kasus->nomor_kasus)->admin == 1) @php $my_role_admin = 1 @endphp
                        ...
                        <div class="block rounded @if(!$loop->first) block-mode-hidden @endif">
                            ...
                            <div class="block-header">
                                <h3 class="block-title">Tagihan #{{$tagihan->id}} - (Rp {{number_format($tagihan->total_sum,0)}})</h3>
                            ...
                            <div class="block-content">
                                ...
                                @if((\Illuminate\Support\Facades\Auth::user()->profesi == 20 || \Illuminate\Support\Facades\Auth::user()->id == 3) && $tagihan->is_paid == 0)
                                    ...
                                    @if(!$tagihan->checkout)
                                    <button class="btn-alt btn-rounded btn-warning min-width-125 float-right" onclick="showModalPindahTagihan({{$tagihan->id}})"><i class="fa fa-random"></i> Pindahkan Tagihan</button>
                                    @endif
                                @endif
                                @if($tagihan->show_sync)
                                <button class="btn-alt btn-rounded btn-primary min-width-125" onclick="showModalSyncTagihan({{$tagihan->id}})"><i class="fa fa-sync"></i> Sinkronisasi Tagihan</button>
                                @endif
                            ...
                            <div class="block-content">
                                ...
                                <div class="table-responsive detail-tagihan-{{ $tagihan->id }}">
                                    <table class="table table-striped table-vcenter">
                                        ...
                                        <tbody>
                                            ...
                                            <tr data-detail-id="{{ $detail->id }}">
                                                ...
                                                <td class="h5 font-w400 text-center">{{$detail->qty or '-'}}</td>
                                            ...
                                            <tr class="hide-element">
                                                ...
                                                <td colspan="2">
                                                    <h4 class="mb-5 mt-5">Rp <span>{{number_format($tagihan->total_sum,0)}}</span></h4>
                                                </td>
                                            </tr>
                                    ...
                                    @if(!$tagihan->checkout)
                                    @if($my_role)
                                    <button class="btn-alt btn-grass text-uppercase mb-10 mb-20 hide-element pull-right" type="button" onclick="showModalCheckout({{$tagihan->id}})">Checkout</button>
                                    @endif
                                    @else
                                    @if($my_role && $tagihan->is_paid == 0)
                                    <button class="btn-alt btn-danger text-uppercase mb-10 mb-20 pull-right hide-element" type="button" onclick="tolakTransaksi({{$tagihan->id}})">Batalkan Checkout</button>
                                    @elseif($tagihan->is_paid == 1)
                                    <span class="badge badge-success hide-element"> <i class="fa fa-check mr-5"></i> Tagihan Telah Dibayar </span><br>
                                    @endif
                                ...
                                @php
                                    $arr_warning = [];
                                    if($kasus->penunjangRadiologi->where('result_created_at',null)->where('kirim_kasir',0)->count() > 0){
                                        $arr_warning[] = "Penunjang Radiologi Belum Terkonfirmasi";
                                    }
                                    if($kasus->penunjang_labpk->where('result_created_at',null)->where('kirim_kasir',0)->count() > 0){
                                        $arr_warning[] = "Penunjang LAB PK Belum Terkonfirmasi";
                                    }
                                    $kasus_transaksi_belum_konfirm = $kasus->transaksiObat->where('status_kasir',0)->where('dikerjakan_at',null);
                                    if($kasus_transaksi_belum_konfirm->count() > 0){
                                        foreach ($kasus_transaksi_belum_konfirm as $item_transaksi) {
                                            $arr_warning[] = 'Resep Obat dari farmasi <b>'.($item_transaksi->owner_detail->nama ?? 'tidak ditemukan mohon melakukan alih resep terlebih dahulu melalui tautan tersebut').'</b> dengan nomor resep <a href="javascript:void(0)">'.($item_transaksi->no_resep ?? 'Tidak ditemukan').'</a> Belum Terkonfirmasi';
                                        }
                                    }
                                @endphp
    ...
    var initAutoCompleteIndex = function(){
        ...
        jQuery('.tagihan-autocomplete').autoComplete({
            ...
            renderItem: function (item, search){
                ...
                content = '<div class="autocomplete-suggestion border-bottom" data-harga="'+item['harga']+'" data-id="'+item['id']+'" data-tarif_id="'+item['tarif_id']+'" data-tarif_master_id="'+item['tarif_master_id']+'" data-persen="'+item['persen']+'" data-val="'+value+'">'
                content+= '<div class="autocomplete-content-top">'+item['deskripsi']+'</div>'
                content+= '<div class="autocomplete-content-bottom text-muted">'+item['kategori_all']+'</div>'
                content+= '</div>'
                return content;
            },
...
<script type="text/javascript">
    ...
    $('#submitCheckout').click(function(event){
        if($('#selectKasir').val() == ''){
            ...
            $('#error-kasir').remove();
            $('#selectKasir').parent().append("<div class='invalid-feedback' id='error-kasir'>Pilih Kasir Terlebih Dahulu</div>");
    ...
    function showModalEdit(id)
    {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#tagihanEditTanggalTransaksi').val(result_created_at[2]+'-'+result_created_at[1]+'-'+result_created_at[0]);
    ...
    function showModalPindahTagihan(tagihan_id) {
        ...
        if (option.length == 0) option.push({ id : "0", text : "Tujuan tagihan tidak tersedia"});
