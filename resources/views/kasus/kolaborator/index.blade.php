                                        ...
                                        <div class="row">
                                            @if (session('my_invitation_' . $kasus->nomor_kasus) && session('my_invitation_' . $kasus->nomor_kasus)->invitation == 1)
                                                <div class="col-lg-12 mb-10">
                                                    <button type="button"
                                                        class="btn-alt btn-primary min-width-125 float-right"
                                                        data-toggle="modal" data-target="#modalTambahKolaborator"><i
                                                            class="fa fa-plus"></i> Tambah Kolaborator </button>
                                        ...
                                        <div class="row">
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="block border">
                                                            <div class="block-content block-content-full clearfix">
                                                                @if (
                                                                    !empty(session('my_invitation_' . $kasus->nomor_kasus)) &&
                                                                    ...
                                                                    @if (session('my_invitation_' . $kasus->nomor_kasus) &&
                                                                        ...
                                                                        @if (!$item->admin)
                                                                            <button type="button"
                                                                                class="btn btn-sm btn-circle btn-alt-danger mr-5 mb-5 float-right"
                                                                                onclick="removeKolaborator({{ $item->id }})"
                                                                        ...
                                                                        @if (
                                                                            !$item->admin &&
                                                                                ($item->user->profesi == 1 || $item->user->profesi == 4) &&
                                                                                session('my_invitation_' . $kasus->nomor_kasus) &&
                                                                                session('my_invitation_' . $kasus->nomor_kasus)->invitation == 1 &&
                                                                                (session('my_invitation_' . $kasus->nomor_kasus)->admin == 1 || Auth::user()->admin == 1))
                                                                            <button type="button"
                                                                                class="btn btn-sm btn-circle btn-alt-info mr-5 mb-5 float-right"
                                                                                onclick="beAdmin({{ $item->user_id }})"
                                                                                data-toggle="tooltip" data-placement="top"
                                                                                title="Jadikan DPJP">
                                                        ...
                                                        <i class="fa fa-user-md"></i>
                                                                    ...
                                                                    <button type="button"
                                                                        class="btn btn-sm btn-circle btn-alt-secondary mr-5 mb-5 float-right print-modal"
                                                                        data-toggle="modal" data-target="#modal-print"
                                                                        data-id="{{ $item->user_id }}">
                                                                        <i data-toggle="tooltip" data-placement="top"
                                                                            title="Print" class="fa fa-print"></i>
                                                                    </button>
                                                                @endif
                                                                <div class="text-left mt-10 ">
                                                                    ...
                                                                    <div class="font-size-sm text-muted">
                                                                        {{ $item->user->profesi_detail->title ?? '-' }}

                                                                        @if ($item->admin)
                                                                            <span class="badge badge-primary">DPJP</span>
                                                                        @endif
                                                                    </div>
                                                                </div>
                                            ...
                                            @empty
                                                ...
                                                <div class="col-12 text-center py-50">
                                                    ...
                                                    <p>Klik tombol <b>Tambah Kolaborator</b> untuk menambah kolaborator pada
                                                        kasus ini</p>
                                                </div>
                                            @endforelse
                                        </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (!$.trim(data)) {
                        ...
                        content +=
                            '<div class="text-center mt-10 "><div class="font-w600 mb-5">User tidak dapat ditemukan</div></div>'
                        content += '</div></div>'
                        ...
                        $.each(data, function(i) {
                            content =
                                '<div class="block border mb-0"><div class="block-content py-20 mb-5 row">'

                            if (data[i].profesi_detail == null) {
                                content +=
                                    '<div class="text-left mt-10 col"><div class="font-w600 mb-5">' +
                                    data[i].name +
                                    '</div><div class="font-size-sm text-muted">-</div></div>'
                            } else {
                                content +=
                                    '<div class="text-left mt-10 col"><div class="font-w600 mb-5">' +
                                    data[i].name + '</div><div class="font-size-sm text-muted">' + data[
                                        i].profesi_detail.title + '</div></div>'
                            }

                            if (data[i].invited == 0) {

                                content +=
                                    '<div class="col-4"><textarea class="form-control" id="pesan' +
                                    data[i].id + '" name="pesan' + data[i].id +
                                    '" rows="2" placeholder="Pesan..."></textarea></div>';

                                content +=
                                    '<button type="button" class="col-3 btn btn-info mr-5 mb-5 float-right done-undang" id="buttonInviteDisabled' +
                                    data[i].id +
                                    '" disabled style="display:none"><i class="fa fa-check"></i> Telah Diundang</button>';


                                content +=
                                    '<button type="button" class="col-3 btn btn-primary mr-5 mb-5 float-right" onclick="buttonClickUser(' +
                                    data[i].id + ')" id="buttonInvite' + data[i].id +
                                    '">Undang Rekan</button>';
                            } else {
                                content +=
                                    '<button type="button" class="col-3 btn btn-primary mr-5 mb-5 float-right" onclick="buttonClickUser(' +
                                    data[i].id + ')" id="buttonInvite' + data[i].id +
                                    '" style="display:none">Batal Undang</button>';
                                content +=
                                    '<button type="button" class="col-3 btn btn-info mr-5 mb-5 float-right done-undang" id="buttonInviteDisabled' +
                                    data[i].id +
                                    '" disabled><i class="fa fa-check"></i> Telah Diundang</button>';
                            }
