    ...
    <div class="row">
                ...
                <div class="dropdown float-right">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal"
                            data-target="#modal-gawat-darurat">
                            <i class="si si-note mr-5"></i> Gawat Darurat
                        </a>
                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal"
                            data-target="#modal-rawat-jalan">
                            <i class="si si-note mr-5"></i> Rawat Jalan
                        </a>
                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal"
                            data-target="#modal-rawat-inap">
                            <i class="si si-note mr-5"></i> Rawat Inap
                        </a>
                        <a class="dropdown-item" href="javascript:void(0)" data-toggle="modal"
                            data-target="#modal-form-triage">
        ...
        <div class="col-lg-12 mb-5 pt-20">
            ...
            @foreach ($asesmen as $item)
                @if ($item['creator']['profesi'] == '2' || $item['creator']['profesi'] == '28')
                    ...
                    @if (
                        ($slug_specialty_creator == 'perawat-vokasi' &&
                            ...
                            $slug_specialty_creator == 'magister-keperawatan')
            ...
            @foreach ($form_triage as $item)
                @if ($item->creator->profesi == '2')
                    @if ($item->created_by == Auth::user()->id)
                        <button class="btn btn-sm btn-circle btn-outline-danger mr-5 mb-5 pull-right deleteFormTriage"
                            data-id="{{ $item->id }}">
                            <i class="fa fa-trash"></i>
                        </button>
                        <button class="btn btn-sm btn-circle btn-outline-primary mr-5 mb-5 pull-right editFormTriage"
                            data-id="{{ $item->id }}" data-item="{{ json_encode($item) }}"
                            id="edit-button-{{ $item->id }}">
                            <i class="fa fa-pencil"></i>
                        </button>
                    ...
                    @endif
                    <button class="btn btn-sm btn-circle btn-outline-secondary mr-5 mb-5 pull-right" data-toggle="modal"
                        data-target="#modal-hasil-alatbantu-{{ $item->id }}">
                        <i class="fa fa-search"></i>
                    </button>
                    <a href="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/form-triage/print/{{ $item->id }}"
                        target="_blank" type="btn"
                        class="btn btn-sm btn-circle btn-outline-secondary mr-5 mb-5 pull-right">
                        <i class="fa fa-print"></i>
                    </a>
                    @php $need_verifikasi_dokter = 0 @endphp
...
<form method="POST" action="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/datamedis/asesmenawal/delete"
    id="formDeleteAsesmen">
    {{ csrf_field() }}
    <input name="id" type="hidden" id="deleteAsesmenId">
</form>
<form method="POST" action="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/form-triage/delete"
    id="formDeleteFormTriage">
    {{ csrf_field() }}
