<div class="content pt-0">
    ...
    @php
        ...
        } elseif (in_array($status_covid, ['kontak erat'])) {
            $covid_banner['tipe_bg'] = 'info';
            $covid_banner['title'] = 'Pasien Kontak Erat Covid-19';
            $covid_banner['subtitle'] = 'Berhati hati saat merawat pasien';
        } elseif (in_array($status_covid, ['meninggal', 'kematian'])) {
        ...
        } elseif (in_array($status_covid, ['pelaku perjalanan'])) {
            $covid_banner['tipe_bg'] = 'elegance-light';
            $covid_banner['title'] = 'Pasien Memiliki Riwayat Perjalanan Covid-19';
            $covid_banner['subtitle'] = 'Berhati hati saat merawat pasien';
        } elseif (in_array($status_covid, ['discarded', 'negatif'])) {
            $covid_banner['tipe_bg'] = 'success';
            $covid_banner['title'] = 'Negatif Covid-19';
            $covid_banner['subtitle'] =
                'Pasien telah didiagnosa negatif. Namun tetap berhati hati saat merawat pasien.';
        } elseif (in_array($status_covid, ['konfirmasi', 'positif'])) {
        ...
        } elseif (in_array($status_covid, ['probable', 'odp', 'pdp', 'otg'])) {
            $covid_banner['tipe_bg'] = 'warning';
    ...
    @includeWhen(
        $status_covid != 'none',
        'kasus.datamedis.content.cppt.components.warning-cppt-covid',
        $covid_banner)
    ...
    <div class="row">
        <div class="col-lg-12 mb-20">
            @if ($allow_crud)
                @if (session('my_invitation_' . $kasus->nomor_kasus)->user->profesi == 10)
                    <button type="button" class="btn-alt btn-primary min-width-125 float-right" onclick="adimeCreate()"><i
                            class="fa fa-pencil"></i> Buat ADIME Gizi</button>
                @else
                    <button type="button" class="btn-alt btn-primary min-width-125 float-right" onclick="cpptCreate()"><i
                            class="fa fa-pencil"></i> Buat CPPT</button>
                @endif
                @if (count($cppts) <= 100)
                    <div class="dropdown">
                        <button type="button" class="btn-alt btn-secondary min-width-125 float-right"
                            id="print-rekap-cppt" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="fa fa-print"></i> Cetak Rekap CPPT <i class="fa fa-angle-down ml-5"></i>
                        </button>
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="print-rekap-cppt"
                            x-placement="bottom-end">
                            <a href="{{ url('kasus') }}/{{ $kasus->nomor_kasus }}/datamedis/cppt/print-all"
                                target="_blink" class="dropdown-item"><i class="fa fa-print"></i> Cetak Semua CPPT</a>
                            <a href="{{ url('kasus') }}/{{ $kasus->nomor_kasus }}/datamedis/cppt/print-sebagian"
                                target="_blink" class="dropdown-item"><i class="fa fa-print"></i> Cetak Sebagian</a>
                        </div>
                    </div>
                @else
                    @php
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="print-rekap-cppt"
                            ...
                            @foreach ($cppt_chunk as $index => $item)
                                ...
                                <a class="dropdown-item"
                                    href="{{ url('kasus') }}/{{ $kasus->nomor_kasus }}/datamedis/cppt/print-all?ids={{ implode('%amp', $item) }}"
                                    target="_blink">
                ...
                @endif
            @endif
            <button type="button" id="cppt-histori-button" class="btn-alt btn-warning  min-width-125 float-right"><i
                    class="fa fa-loop"></i> Histori CPPT</button>
            <div class="dropdown">
                <button type="button" class="btn-alt btn-secondary min-width-125 float-right full-only"
                    id="page-header-user-dropdown-full" data-toggle="dropdown" aria-haspopup="true"
                    aria-expanded="false">
                    Print<i class="fa fa-angle-down ml-5"></i>
                </button>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="page-header-user-dropdown-full"
                    x-placement="bottom-end">
                    <a class="dropdown-item" href="javascript:void(0)"
                        onclick="popupwindow('{{ url('') }}/pasien/{{ $kasus->pasien->id }}/print/ringkasan-ranap', 'Print', 800, 600)">
                        <i class="fa fa-file-text mr-5"></i> Riwayat Rawat Inap
                    </a>
                    <a class="dropdown-item" href="javascript:void(0)"
                        onclick="popupwindow('{{ url('') }}/pasien/{{ $kasus->pasien->id }}/print/ringkasan-rajal', 'Print', 800, 600)">
                        <i class="fa fa-file-text mr-5"></i> Riwayat Rawat Jalan
                    </a>
                    <a class="dropdown-item" href="javascript:void(0)"
                        onclick="popupwindow('{{ url('') }}/pasien/{{ $kasus->pasien->id }}/print/prmrj', 'Print', 800, 600)">
        ...
        @if (!empty($unread_readback))
            <div class="col-md-12">
                <div class="block block-bordered card bg-danger p-2" style="color:white; font-weight: 500">ada
                    {{ $unread_readback }} cppt belum di readback</div>
            </div>
        @endif
            ...
            <div class="col-md-12">
                <div class="block block-bordered block-mode-hidden">
                    <div class="block-header block-header-default"> <!-- parent btn-edit-cppt -->
                        <h3 class="block-title d-flex"><span
                                style="white-space: nowrap;margin-right: .75rem">{{ $cppt->jenis ? strtoupper($cppt->jenis) : 'CPPT' }}
                                {{ $i }}</span>
                            @php
                            ...
                            <small style="white-space: nowrap;margin-right: .75rem">
                                ...
                                @if ($cppt->creator && $cppt->creator->profesi == 1)
                                    @php $class_cppt = 'badge badge-primary' @endphp
                                @elseif($cppt->creator && $cppt->creator->profesi == 2)
                                    @php $class_cppt = 'badge badge-success' @endphp
                                @elseif($cppt->creator && $cppt->creator->profesi == 3)
                                    @php $class_cppt = 'badge badge-danger' @endphp
                                @elseif($cppt->creator && $cppt->creator->profesi == 10)
                                    @php $class_cppt = 'badge badge-warning' @endphp
                                @else
                                    @php $class_cppt = 'badge badge-secondary' @endphp
                                @endif
                            ...
                            <small style="white-space: normal">
                                @if (Auth::user()->profesi == 2 && !empty(session('my_role_' . request()->route('nomor_kasus'))))
                                    @forelse ($cppt->readbacks as $readback)
                                        ...
                                        @endphp
                                        <span class="{{ $class_readback }}">{{ $user_readback->name }} -
                                            {{ $readback->verified_at . ' Read Back' ?? 'Belum Verifikasi' }}</span>
                                    @empty
                        ...
                        <div id="marked-print-cppt-{{ $i }}">
                            <button type="button" class="btn-block-option"
                                title=" {{ $sudah_termarked_print ? 'Hapus ' : '' }} Marked Print"
                                onclick="markedPrintCPPT( {{ $cppt->id }} , {{ $i }}, {{ $sudah_termarked_print }} )">
                        ...
                        @if ($allow_crud == 1 && $cppt->created_by == Auth::user()->id && !$cppt->jenis)
                            <button type="button" class="btn-block-option" data-toggle="tooltip"
                                data-placement="top" title="Copy CPPT" onclick="cpptCopy({{ $cppt->id }})">
                                <i class="fal fa-copy"></i>
                            </button>
                        @endif

                        @if (Auth::user()->profesi == 2 &&
                                !empty(session('my_role_' . request()->route('nomor_kasus'))) &&
                                $cppt->readbacks()->count() == 0)
                            <button type="button" class="btn-block-option" data-toggle="tooltip"
                                data-placement="top" title="Readback" onclick="readback({{ $cppt->id }})">
                                <i class="far fa-file-alt"></i>
                            </button>
                        @endif
                        @if (!empty($my_readback) && $my_readback->verified_at == null)
                            <button type="button" class="btn-block-option" data-toggle="tooltip"
                                data-placement="top" title="Verifikasi Readback"
                                onclick="verifReadback({{ $my_readback->id }})">
                                <i class="far fa-file-alt"></i>
                            </button>
                        @endif

                        @if (isset($cppt->jenis))
                            @if ($cppt->jenis == 'rapt' || $cppt->jenis == 'adime')
                                <button type="button" class="btn-block-option" data-toggle="tooltip"
                                    data-placement="top" title="Print CPPT"
                                    onclick="raptPrint({{ $cppt->id }})">
                                    <i class="si si-printer"></i>
                                </button>
                            @endif
                        @else
                            <button type="button" class="btn-block-option" data-toggle="tooltip"
                                data-placement="top" title="Print" onclick="cpptPrint({{ $cppt->id }})">
                                <i class="si si-printer"></i>
                            </button>
                        @endif
                        ...
                        @if (
                            $allow_crud == 1 &&
                                $cppt->jenis != 'adime' &&
                                in_array(Auth::user()->id, explode(',', $cppt->creator->user_allow_override)))
                            <button type="button" class="btn-block-option" data-toggle="tooltip"
                                data-placement="top" title="Override"
                                onclick="cpptOverrideModal({{ $cppt->id }},{{ $i }})">
                        ...
                        @if ($slug_specialty_creator == 'perawat-vokasi')
                            @if (empty($cppt->verified_at) &&
                                    $my_role &&
                                    Auth::user()->profesi == 1 &&
                                    !in_array(Auth::user()->specialty, [1, 2, 90]) &&
                                    Auth::user()->id != $cppt->created_by)
                        ...
                        @else
                            @if (empty($cppt->verified_at) &&
                                    $my_role_admin == 1 &&
                                    Auth::user()->id != $cppt->created_by &&
                                    $cppt->creator->profesi == 1 &&
                                    !in_array($cppt->creator->specialty, [1, 2]))
                                @php $need_verifikasi_dokter = 1 @endphp
                            @elseif(empty($cppt->verified_at) &&
                                    $my_role &&
                                    Auth::user()->profesi == 1 &&
                                    !in_array(Auth::user()->specialty, [1, 2]) &&
                                    Auth::user()->id != $cppt->created_by)
                                @php $need_verifikasi_dokter = 1 @endphp
                            @endif
                        @endif
                        ...
                        @if ($need_verifikasi_dokter)
                            <button type="button" id="cppt_button_verifikasi_{{ $i }}"
                                class="btn-block-option" data-toggle="tooltip" data-placement="top"
                                title="Verifikasi"
                                onclick="cpptVerifikasi({{ $cppt->id }},{{ $i }})">
                                <i class="si si-check" id="cppt_button_verifikasi_check_{{ $i }}"></i>
                                <span id="cppt_button_verifikasi_loading_{{ $i }}" class="hide"><i
                                        class="fa fa-spinner fa-spin"></i> Verifying</span>
                            </button>
                        @endif
                        ...
                        @if (empty($cppt->review) && $my_role_admin == 1 && $cppt->created_by != Auth::user()->id)
                            <button type="button" class="btn-block-option" data-toggle="tooltip"
                                ...
                                onclick="cpptReviewModal({{ $cppt->id }},{{ $i }})">
                                <i class="si si-eyeglasses"></i>
                            </button>
                        @endif

                        @if (empty($cppt->verified_ners_at) &&
                                $slug_specialty_creator == 'perawat-vokasi' &&
                                $slug_specialty_user == 'perawat-ners')
                            <button type="button" id="cppt_button_verifikasi_ners_{{ $i }}"
                                class="btn-block-option" data-toggle="tooltip" data-placement="top"
                                title="Verifikasi"
                                onclick="cpptVerifikasiNERS({{ $cppt->id }},{{ $i }})">
                                <i class="si si-check" id="cppt_button_verifikasi_check_{{ $i }}"></i>
                                <span id="cppt_button_verifikasi_ners_loading_{{ $i }}" class="hide"><i
                                        class="fa fa-spinner fa-spin"></i> Verifying</span>
                            </button>
                        @endif
                        ...
                        @if ($allow_crud == 1 && $cppt->created_by == Auth::user()->id)
                            <button type="button" class="btn-block-option " data-toggle="tooltip"
                                ...
                                onclick="cpptDeleteModal({{ $cppt->id }},{{ $i }})">
                                <i class="si si-trash"></i>
                            </button>

                            @if ($cppt->jenis == 'adime')
                                <button type="button" class="btn-block-option" data-toggle="tooltip"
                                    data-placement="top" title="Edit" onclick="adimeEdit({{ $cppt }})">
                                    <i class="si si-pencil"></i>
                                </button>
                            @else
                                <button type="button" class="btn-block-option btn-edit-cppt" data-toggle="tooltip"
                                    data-placement="top" title="Edit"
                                    onclick="cpptEditModal({{ $cppt->id }},{{ $i }})">
                        ...
                        <div class="block-options">
                            <button type="button" class="btn-block-option btn-content-toogle"
                                data-status="non-active" data-toggle="block-option"
                        ...
                        @else
                            @if ($cppt->jenis == 'rapt')
                                <div class="row">
                                    ...
                                    <div class="col-4">
                                        ...
                                        <h5 class="font-w400" style="white-space: pre-line">
                                            @if ($cppt->prioritas == 'prioritas')
                                                <strong>Prioritas</strong>
                                            @elseif($cppt->prioritas == 'tunda')
                                                Dapat Ditunda
                                            @endif
                                        </h5>
                            ...
                            @if (!empty($files))
                                ...
                                <p style="text-align: center">{{ $value->nama_file }}</p>
                                        ...
                                        <a href="" style="display:block;width:130px" data-toggle="modal"
                                            data-target="#preview-modal-{{ $loop_iteration_cppt }}-{{ $loop->iteration }}">
                                            <img src="{{ URL::asset('assets/img/filetype.png') }}" align="middle"
                                                style="max-width: 100px;display: block;margin-left: auto;margin-right: auto">
                                            <p style="text-align: center">{{ $value->nama_file }}</p>
                                        </a>
                                        @if ($allow_crud == 1 && $cppt->created_by == Auth::user()->id)
                                            <button type="button" style="height: 50px;" class="btn-block-option "
                                                data-toggle="tooltip" data-placement="top" title="Hapus"
                                                onclick="cpptDeletefileModal({{ $cppt->id }},{{ $value->id }})">
                            ...
                            @if (!empty($cppt->discharge_planning))
                                ...
                                @include('kasus.datamedis.content.asesmenawal.components.view-pulang', [
                                    'item' => $item,
                                ])
                            @endif
                        @endif
                                ...
                                <div class="col-4 hide" id="cppt_ners_container_verified_{{ $i }}">
                                    <h6 class="p-10">
                                        ...
                                        <span class="font-w400"
                                            id="cppt_ners_verified_at_{{ $i }}"></span>
                                    </h6>
                                </div>
                            @endif
