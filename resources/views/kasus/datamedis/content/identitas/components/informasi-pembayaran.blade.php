<div class="col-lg-12 mb-5">
    @if ($allow_crud)
        <a type="button" class="btn-alt btn-primary min-width-125 float-right"
            href="{{ url('kasus') }}/{{ $kasus->nomor_kasus }}/datamedis/identitas/pembayaran/update"><i
                class="fa fa-pencil"></i> Edit Pembayaran</a>
    @endif
    @if (!empty($kasus->active_sep))
        <button class="btn-alt btn-info min-width-125 float-right" onclick="printSEP()">Print SEP</button>
        <button class="btn-alt btn-secondary min-width-125 float-right" onclick="printSEPwithBuktiPelayanan()"><i
                class="fas fa-print"></i> SEP + Bukti Pelayanan</button>
        <button class="btn-alt btn-warning width-125 float-right" onclick="syncSEP()"><i class="fas fa-sync"></i>
            Sync SEP</button>
    @endif
