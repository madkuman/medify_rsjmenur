							...
							<div class="block-content px-20 pt-50">
								...
								@if($is_kolaborator)
									@if(count($pengobatan) > 0)
										...
										@if (config('medify.kasus.catatan_pemberian_obat.form_pemberian_obat') == "1")
                                            <button type="button" class="btn btn-secondary min-width-125 pull-right  mr-10 isiPemberianObatBtnSingle" data-method="create" data-id=""><i class="fa fa-plus mr-5"></i>Isi Pemberian</button>
                                        @else
                                            <button type="button" class="btn btn-secondary min-width-125 pull-right  mr-10 isiPemberianObatBtn" data-method="create" data-id=""><i class="fa fa-plus mr-5"></i>Isi Pemberian</button>
                                        @endif
									@endif
									...
									@if(count($pengobatan) > 0)
										<button type="button" class="btn btn-secondary pull-right dropdown-toggle mr-2" id="page-header-options-dropdown2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
											<i class="fa fa-print mr-2" aria-hidden="true"></i>Cetak
										</button>
										<div class="dropdown-menu" aria-labelledby="page-header-options-dropdown2">
											<a class="dropdown-item" href="{{url()->current()}}/print" target="_blank" style="cursor: pointer;">
												<i class="fa fa-clipboard mr-2" aria-hidden="true"></i>Cetak Pengobatan Pasien
											</a>
											<button type="button" class="dropdown-item" style="cursor: pointer;" data-toggle="modal" data-target="#modal-cetak-riwayat-pemberian-obat">
												<i class="fa fa-clipboard mr-2" aria-hidden="true"></i>Cetak Riwayat Pemberian Obat
											</button>
										</div>
									@endif
								@endif
								...
								<table class="table table-bordered table-vcenter table-responsive" style="width: 100%;">
									...
									<tbody>
										...
										<tr>
											<td>{{$count++}}</td>
											<td><p style="white-space: pre">{{$item->nama_obat ?? "-"}}</p>
											...
											<td>
												...
												<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
													...
													<a class="dropdown-item deleteObatBtn" href="javascript:void(0)" data-id="{{$item->id}}" disabled="{{$disabled}}">
														<i class="fa fa-trash mr-5"></i>Hapus														
													</a>					
													@else
													<a class="dropdown-item" >
														<i class="fa fa-info mr-5"></i>Anda Bukan Pembuat Item Ini														
													</a>
													@endif
...
<script type="text/javascript">
	...
	$(".save-pemberian-obat" ).click(function( e ) {
		...
		if ($('.input-select-obat').val().length == 0) {
			...
			return 0;
		}else { $('#error-nama-pemberian-obat').addClass('d-none'); }
...
<script type="text/javascript">
	...
	$("#form-pemberian-obat" ).submit(function( e ) {
		...
		formData.push({name: 'imgUrl', value: imgUrl});
		formData.push({name: 'imgUrl2', value: imgUrl2});
