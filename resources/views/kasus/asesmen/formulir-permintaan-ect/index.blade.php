			...
			<div class="col-lg-9 col-xl-9">
				<div class="block block-bordered">
					<div class="block-content">
						...
						@endphp
						@if (($data_val->ttd_yang_menyatakan_persetujuan ?? null) == null || ($data_val->ttd_yang_menyatakan_penolakan ?? null) == null)
							<div class="d-flex justify-content-end w-100">
								@empty($data_val->ttd_yang_menyatakan_persetujuan)
									<button id="btnTtdYangMenyatakanPersetujuan-{{$hasil_item->id}}" data-for="Yang Menyatakan Persetujuan" type="button" data-ttd-type="ttd_yang_menyatakan_persetujuan" class="btn btn-primary mr-5 align-middle btn-add-ttd" data-id="{{$hasil_item->id}}" title="Tambah Tanda Yang Menyatakan Persetujuan" data-original-title="Tambah Tanda Tangan Yang Menyatakan Persetujuan">
										<i class="fas fa-file-signature"></i>
										<span>TTD Yang Menyatakan Persetujuan</span>
									</button>
								@endempty
								@empty($data_val->ttd_yang_menyatakan_penolakan)
									<button id="btnTtdYangMenyatakanPenolakan-{{$hasil_item->id}}" data-for="Yang Menyatakan Penolakan" type="button" data-ttd-type="ttd_yang_menyatakan_penolakan" class="btn btn-primary mr-5 align-middle btn-add-ttd" data-id="{{$hasil_item->id}}" title="Tambah Tanda Yang Menyatakan Penolakan" data-original-title="Tambah Tanda Tangan Yang Menyatakan Penolakan">
										<i class="fas fa-file-signature"></i>
										<span>TTD Yang Menyatakan Penolakan</span>
									</button>
								@endempty
							</div>
						@endif
...
<script type="text/javascript">
    $(".btn-add-ttd").click(function(e){
		...
		$("#nama_yang_menyatakan").val(namaPasien)
	...
	$('#btnSubmitTtd').on( 'click', function() {       
		...
		var yangMenyatakan = $("#nama_yang_menyatakan").val();
		...
		formDataTtd.append('yang_menyatakan', yangMenyatakan);
		...
		$.ajax({
			...
			success: function(response) {
				...
				if(ttdType == 'ttd_yang_menyatakan_persetujuan') {
					$(`#btnTtdYangMenyatakanPersetujuan-${ttd_id}`).hide(); 
				} else if(ttdType == 'ttd_yang_menyatakan_penolakan') {
					$(`#btnTtdYangMenyatakanPenolakan-${ttd_id}`).hide(); 
				}
				$("#addTtd").modal("toggle");
