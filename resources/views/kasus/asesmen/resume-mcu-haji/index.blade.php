...
@section('content')
                    ...
                    <div class="block block-bordered">
                        <div class="block-content">
                            @if (session('my_role_' . $kasus->nomor_kasus))
                                <a href="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/asesmen/{{ $form->slug }}/create"
                                    class="btn btn-rounded btn-alt-primary min-width-125 float-right"><i
                                        class="fa fa-pencil"></i> {{ $form->nama_show ?? '' }} </a>
                            @endif
                            ...
                            @forelse($hasil as $hasil_item)
                                @if (session('my_role_' . $kasus->nomor_kasus))
                                    @if ($hasil_item->created_by == Auth::user()->id)
                                        <button
                                            ...
                                            data-id="{{ $hasil_item->id }}">
                                            <i class="fa fa-trash"></i>
                                        </button>
                                    @endif
                                @endif
                                <a href="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/asesmen/{{ $form->slug }}/edit/{{ $hasil_item->id }}"
                                    class="btn btn-sm btn-circle btn-outline-warning mr-5 mb-5 pull-right editBtn"
                                    data-id="{{ $hasil_item->id }}" data-index="{{ $loop->iteration - 1 }}">
                                    <i class="fa fa-pencil"></i>
                                </a>
                                <a type="btn"
                                    href="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/asesmen/{{ $form->slug }}/print/{{ $hasil_item->id }}"
                                    class="btn btn-sm btn-circle btn-outline-secondary mr-5 mb-5 pull-right"
                                    target="_blank">
                                    <i class="fa fa-print"></i>
                                </a>
                                <a href="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/asesmen/{{ $form->slug }}/view/{{ $hasil_item->id }}"
                                ...
                                {{-- @if (($data_val->ttd_persetujuan_pasien ?? null) == null)
                                    ...
                                    @empty($data_val->ttd_persetujuan_pasien)
                                        <button id="btnTtdPersetujuanPasien-{{ $hasil_item->id }}"
                                            data-for="Persetujuan Pasien" type="button"
                                            data-ttd-type="ttd_persetujuan_pasien"
                                            class="btn btn-primary mr-5 align-middle btn-add-ttd"
                                            data-id="{{ $hasil_item->id }}" title="Tambah Tanda Persetujuan Pasien"
                                            data-original-title="Tambah Tanda Tangan Persetujuan Pasien">
                                ...
                                @if (!empty($hasil_item->creator->avatar_thumb))
                                    <div class="float-left mr-10">
                                        <img class="img-avatar img-avatar-sm img-avatar-thumb"
                                            src="{{ url($hasil_item->creator->avatar_thumb) }}" alt="">
                                    </div>
                                @else
                                    <div class="float-left mr-10">
                                        <img class="img-avatar img-avatar-sm img-avatar-thumb"
                            ...
                            @empty
                                ...
                                <div class="text-center py-50">
                                    <h4 class="font-w400 mb-5">Belum ada Resume MCU Haji {{ $form->nama_show ?? '' }}
                                        tersedia</h4>
                                    <p>Klik tombol <b>{{ $form->nama_show ?? '' }} Baru</b> untuk membuat Resume MCU Haji
                                        {{ $form->nama_show ?? '' }}</p>
                                </div>
                            @endforelse
                        </div>
    ...
    <form method="POST" action="{{ url('') }}/kasus/{{ $kasus->nomor_kasus }}/asesmen/{{ $form->slug }}/delete"
        id="formDelete">
        {{ csrf_field() }}
