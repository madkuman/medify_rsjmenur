...
@section('title')
    {{ $kasus->judul_kasus }} - Print Asesmen Resiko Kekerasan Fisik
@endsection
...
@section('content')
    ...
    <table class="ms-table-bordered ms-border-bottom-0">
        ...
        <tr>
            ...
            @for ($i = 0; $i < 7; $i++)
                @if ($i === 0)
                    <td colspan="2"></td>
                @else
                    <td colspan="3" class="ms-text-center">
                        {{ \Carbon\Carbon::parse($alatbantu->val->tanggal[$i - 1])->format('d/m/Y') }}
                    </td>
                @endif
            @endfor
        </tr>
        ...
        @php
            $faktor = ['Mengekspresikan ide untuk melukai orang lain', 'Akses melakukan tindak kekerasan', 'Ide Paranoid atau lainnya', 'Perintah Halusinasi untuk melakukan tindak kekerasan', 'Kemarahan, Frustasi, atau Agitasi', 'Kesenangan untuk ide / tindak kekerasan', 'Perilaku seksual tidak wajar', 'Berkurang / kehilangan kontrol diri', 'Penggunaan Napza'];
        ...
        <tr>
            <td colspan="2">LEVEL RISIKO KEKERASAN FISIK Total Skor :</td>
            <td></td>
            <td colspan="18">
    ...
    <table style="page-break-after: always" class="ms-table-bordered ms-border-bottom-0">
