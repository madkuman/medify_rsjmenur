	...
	<div class="content">
		...
		<div class="block">
			<div class="block-content block-content-full">
				<div class="row">
					<div class="col-12">
						<H6>TRANSAKSI PERMINTAAN #{{$transaksi->id}}</H6>
						<hr>
						<div class="row">
							<div class="col">
					...
					<div class="block-content block-content-full">
						...
						<div class="row">
							...
							<div class="col-12" id="edit-container" style="display: none">
								<div class="row">
									<div class="col-12 mb-5">
										<div class="custom-control custom-radio custom-control-inline mb-5">
											<input type="radio" name="scan-mode" class="custom-control-input" id="satuan" value="satuan" checked>
											<label class="custom-control-label" for="satuan">Satuan</label>
										</div>
										<div class="custom-control custom-radio custom-control-inline mb-5">
											<input type="radio" name="scan-mode" class="custom-control-input" id="paket" value="paket">
								...
								<form action="{{url()->current()}}/kirim-alkes" method="POST">
									...
									<div class="form-group mt-50">
										...
										<button class="btn btn-primary">Simpan & Kirimkan Alkes</button>
									</div>
								</form>
							</div>
						</div>
						...
						<form action="{{url()->current()}}/kirim-alkes" method="POST">
							...
							<div class="form-group text-center mt-50">
								<button class="btn btn-primary">Simpan & Kirimkan Alkes</button><br><br>
...
<script type="text/javascript">
	...
	function searchAlkesBarcode()
	{
		...
		if ($('input[name=scan-mode]:checked').val() == 'satuan') {
			searchAlkesSatuan(kode);
		} else {
			searchAlkesPaket(kode);
		}
	}
	...
	function searchAlkesPaket(kode)
	{
		clearTimeout(timer);
		$.ajax({
			type: "GET",
			contentType: "application/json; charset=utf-8",
			dataType: 'json',
			url: API_URL + '/cssd/alkes/paket/get/'+kode,
			success: function (result) {
				if(result == 0)
				{
					swal({
				...
				else
				{
					if (result.error) {
						...
						for (i in result.success) {
			                kode = result.success[i].slug;
							nama = result.success[i].alkes.nama;
							$('#pengiriman-container').append(`
