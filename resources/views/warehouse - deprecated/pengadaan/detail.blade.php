...
@section('content')
    ...
    <div class="card">
        <div class="card-header ">
            <div class="row">
                <div class="col-md-6">
                    <h4 class="card-title">Asal Barang : 
                                @if($transaction->type == 1)
                                    {{$transaction->seller_detail->nama}}
                                @else
                                ...
                                @endif</h4>
                    @if($transaction->type == 1)
                        <p class="card-category">Jenis Transaksi : MASUK</p>
                    @else
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div style="float:right;">{{number_format($item->subtotal)}}</div>
                                    </td>
                                </tr>
                                @endforeach 
                                <tr>
                                    ...
                                    <td>
                                        <div style="float:left;">Rp.</div>
                                        <div style="float:right;">{{number_format($transaction->total_price)}}</div>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div class="card-footer">
            <div class="row">
                {{-- <div class="col-md-6">
                    <div class="pull-left">
                        <form method="POST" action="{{url('warehouse/pengadaan/delete')}}">
                            {{csrf_field()}}
                            <input type="hidden" name="id" value="{{$transaction->id}}">
                        </form>
                        <button type="submit" class="btn btn-danger btn-fill" id="delete-transc">
                            <i class="fa fa-trash-o" aria-hidden="true"></i> Hapus
                        </button>
                        <a href="{{url('warehouse/transaction/edit/'.$transaction->slug)}}" class="btn btn-warning btn-fill"><i class="fa fa-pencil" aria-hidden="true"></i> Ubah</a>
                    </div>
                </div> --}}
                @if($transaction->status==0)
                <div class="col-md-12">
                    <div class="pull-right">
                        <form method="POST" action="{{url('warehouse/pengadaan/edit/'.$transaction->slug)}}">
                            {{csrf_field()}}
                            <input type="hidden" name="id" value="{{$transaction->id}}">
                        </form>
                        <button type="button" class="btn btn-danger btn-fill" id="btnReject">
                            <i class="fa fa-times" aria-hidden="true"></i> Tolak</button>
                        <a href="{{url('warehouse/pengadaan/edit/'.$transaction->slug)}}" class="btn btn-primary btn-fill"><i     class="fa fa-pencil" aria-hidden="true"></i> Verifikasi</a>
                        {{-- <button type="submit" class="btn btn-primary btn-fill">
                            <i class="fa fa-check" aria-hidden="true"></i> Verifikasi</button> --}}
                    </div>
                </div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="pull-right">
                                            <form method="POST" action="{{url('warehouse/pengadaan')}}/accept">
                                                {{csrf_field()}}
                                                <input type="hidden" name="id" value="{{$transaction->id}}">
            ...
            <div class="row reject-form hidden">
                <div class="col-md-12">
                    <form method="POST" action="{{url('warehouse/pengadaan')}}/reject">
                        {{csrf_field()}}
                        <input type="hidden" name="id" value="{{$transaction->id}}">
                    ...
                    <div class="col-md-12 text-center">
                        ...
                        <div class="text-center">
                            <span>
                                ...
                                <strong>{{$transaction->created_by_detail->name}}</strong>, {{ date('d F Y, h:i', strtotime($transaction->updated_at)) }}
                            </span>
                        </div>
                        <div class="center">
                            {{-- <div class="card card-lock text-center card-plain">
                                <div class="card-header ">
                                    <div class="author">
                                        <img src="{{ (!is_null($transaction->created_by_detail->avatar_thumb) ? asset($transaction->created_by_detail->avatar_thumb) : asset('assets/app/warehouse/no_image.png')) }} " class="avatar" onerror="imgError(this);"
                                        >
                                    </div>
                                </div>
                            </div> --}}
...
@section('js')
    <script type="text/javascript">
        ...
        $('#btnReject').on('click', function() {
            var reject = 
                '<form method="POST" action="{{url('warehouse/pengadaan')}}/reject" id="rejectForm">' +
                    '{{csrf_field()}}' +
                    '<input type="hidden" name="id" value="{{$transaction->id}}">' +
                    '<div class="form-container">' +
                        '<div class="form-container">' +
