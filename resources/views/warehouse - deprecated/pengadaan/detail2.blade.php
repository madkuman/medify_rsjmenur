...
@section('content')
    <div class="block">
        <div class="block-header bordered">
            ...
            <div class="block-options">
                <form method="POST" action="{{url('gudang/pengadaan/delete')}}">
                    {{csrf_field()}}
                    <input type="hidden" name="id" value="{{$pengadaan->id}}">
                </form>
                <a href="{{url('gudang/pengadaan/'.$pengadaan->slug.'/print')}}" class="btn btn-alt-warning btn-square" target="_blank">
    ...
    <div class="modal" id="modal-large" role="dialog" aria-labelledby="modal-large" aria-hidden="true">
        <div class="modal-dialog modal-full" role="document">
            <form method="POST" enctype="multipart/form-data" action="{{url('gudang/pengadaan')}}/edit" id="form-pengadaan">
                {{csrf_field()}}
                <input type="hidden" name="id" value="{{$pengadaan->id}}">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <div>
                                            <select  class="form-control js-select2" name="po" data-placeholder="Pilih PO" style="width: 100%;" id="po" @if(isset($pengadaan->po_id)) disabled="" @endif>
                                                <option></option>
                                                <option value="0">PO Baru</option>
                                                @foreach($po_available as $po)
                                                <option value="{{$po->id}}">{{$po->judul}} - {{$po->no_po}}</option>
                                                @endforeach
                                                @if($po_selected)
                                                <option value="{{$po_selected->id}}"
                                                 selected="" >
                                                 {{$po_selected->no_po}}
                                                </option>
                                                 @endif
