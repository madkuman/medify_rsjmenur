...
@section('content')
    ...
    <div class="content">
        <div class="block block-themed">
            <div class="block-header" style="background-color:#ffffff">
                <h3 class="block-title" style="font-weight: 550; color: black">Permintaan Transaksi #{{ $transaksi['id'] }}
                </h3>
                @if ($transaksi->status != -1)
                    <span class="pull-right">
                        <a class="btn btn-hero btn-secondary"
                            href="{{ url('radiologi/transaksi/edit/' . $transaksi->slug) }}"><i
                                class="fa fa-pencil"></i>&nbsp&nbsp&nbspEdit</a>
                    </span>
                @endif
            </div>
            <div class="block-content pb-20">
                <div class="row px-15">
                    ...
                    <div class="modal fade" id="modalConfirmationSEP" tabindex="-1" role="dialog"
                        aria-labelledby="modalConfirmation" style="display: none;" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="block block-themed block-transparent mb-0">
                                    <div class="block-content">
                                        <form action="{{ url('radiologi/transaksi/sep/edit/' . $transaksi->slug) }}"
                                            method="POST">
                                            <h3>Ubah Nomor SEP</h3>
                                            {{ csrf_field() }}
                                            <p>Masukkan Nomor SEP yang baru</p>
                                            {{ Form::label('tanggal_periksa', 'Nomor SEP baru') }}
                                            <input type="number" name="sep_number" class="form-control"
                                                placeholder="Masukkan Nomor SEP baru di sini">
                                            <br>
                                    </div>
                                </div>
                ...
                @if ($is_dokter && !is_null($transaksi->kasus_id) && $transaksi->status == 1)
                    <div class="col-md-12 form-group mt-10">
                        <button class="btn btn-secondary pull-right ml-5 mb-5 wide-mobile" type="button"
                            data-toggle="modal" data-target="#modal-create-tindakan-icd9">Tambah Tindakan ICD9</button>
                    </div>
                @endif
                <div class="col-12">
                    <div>
                        ...
                        <ul>
                            @foreach ($transaksi->detail as $detail)
                                @if ($detail->status == 'done')
                                    <li>
                                        <p class="font-w600 mb-5">{{ $detail->tarif->deskripsi }} &nbsp&nbsp&nbsp <span
                                                style="font-weight: 400;"><br>- {{ $detail->qty }} kali <br>-
                                                {{ $detail->film_dipakai }} Film dipakai <br>- {{ $detail->film_ditolak }}
                                                Film ditolak <br>- Ukuran Film ({{ $detail->ukuran_film }})<br>-
                                                {{ $detail->foto_ditolak }} Foto Ulang
                                                ({{ $detail->alasan_foto_ulang }})<br>- {{ $detail->kontras_dipakai }}
                                                Kontras dipakai<br>- {{ $detail->kontras_dikembalikan }} Kontras
                                                dikembalikan</span></p>
                                        <br><button class="btn btn-hero btn-primary btn-hasil-baca" type="button"
                                            data-title="{{ $detail->tarif->deskripsi }}">Hasil Baca</button>
                                        <input type="hidden" class="hasil-baca" value="{{ $detail->hasil_baca }}">
                                    </li>
                                @endif
                            @endforeach
                        </ul>
                    ...
                    <div class="mobile-block">
                        <ul>
                            @foreach ($transaksi->detail as $detail)
                                @if ($detail->status == 'done')
                                    <li>
                                        <p class="font-w600 mb-5">{{ $detail->tarif->deskripsi }}<span
                                                style="font-weight: 400;"><br>- {{ $detail->qty }} kali <br>-
                                                {{ $detail->film_dipakai }} Film dipakai <br>- {{ $detail->film_ditolak }}
                                                Film ditolak - Ukuran Film ({{ $detail->ukuran_film }})</span></p>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
    ...
    <div class="modal" id="hasil_baca_modal" tabindex="-1" role="dialog" aria-labelledby="modal-large"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="block block-themed block-transparent mb-0">
