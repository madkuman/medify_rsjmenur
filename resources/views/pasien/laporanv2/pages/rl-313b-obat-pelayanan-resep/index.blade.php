...
@section('title')
RL 3.13b Obat Pelayanan Resep - Laporan - Administrasi & Rekam Medis
@endsection
...
<script type="text/javascript">
    ...
    $('.btn-get-data').click(function(){
        ...
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rl-313b-obat-pelayanan-resep/get-total-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
    ...
    function getDataset()
    {
        ...
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rl-313b-obat-pelayanan-resep/get-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
            data : 'datestart='+start_date+'&dateend='+end_date+'&datafetched='+data_fetched+'&limit='+data_per_fetch,
            success:function(results){
                content.push(results.content)
                var last_id = results.last_id
                data_fetched+=data_per_fetch

                if(data_fetched%5 != 0){
                    getDataset()
                    var percentage = Math.ceil(data_fetched / 15 * 100)
