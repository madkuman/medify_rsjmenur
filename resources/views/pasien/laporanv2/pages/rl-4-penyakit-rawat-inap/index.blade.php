...
@section('title')
RL 4 Penyakit Rawat Inap (Sebab Luar) - Laporan - Administrasi & Rekam Medis
@endsection
...
@section('page-title')
RL 4 Penyakit Rawat Inap (Sebab Luar)
@endsection
    ...
    <div class="row">
        ...
        <div class="col-4">
            <div class="form-group">
                ...
                <select class="js-select2 form-control input-bangsal" name="bangsal[]" placeholder="Pilih Bangsal" multiple="multiple" style="width: 100%;">
                    @foreach($bangsal as $bang)
                    <option value="{{$bang->id}}">{{$bang->nama}}</option>
                    @endforeach
                </select>
                <small>Kosongkan untuk melihat data dari semua bangsal</small>
            </div>
        </div>
    ...
    <table class="table table-bordered table-striped table-vcenter js-dataTable-full" id="example">
        <thead>
            <tr>
                ...
                <th>PASIEN KELUAR (HIDUP&MATI) LK</th>
                <th>PASIEN KELUAR (HIDUP&MATI) PR</th>
                <th>JML PASIEN KELUAR HIDUP&MATI (LK+PR)</th>
...
<script type="text/javascript">
    ...
    $('.btn-get-data').click(function(){
        ...
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rl-4-penyakit-rawat-inap/get-total-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
    ...
    function getDataset()
    {
        var dataTableObj = $('.js-dataTable-full').DataTable();
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rl-4-penyakit-rawat-inap/get-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
            data : 'datestart='+start_date+'&dateend='+end_date+'&datafetched='+data_fetched+'&bangsal='+bangsal+'&dtd='+dtd[data_fetched],
            success:function(results){
