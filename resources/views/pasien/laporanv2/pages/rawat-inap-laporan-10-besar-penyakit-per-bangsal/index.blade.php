...
@section('title')
Rawat Inap - Laporan 10 Besar Penyakit Per Bangsal - Laporan - Administrasi & Rekam Medis
@endsection
...
@section('page-title')
Rawat Inap - Laporan 10 Besar Penyakit Per Bangsal
@endsection
    ...
    <table class="table table-bordered table-striped table-vcenter js-dataTable-full" id="example">
        <thead>
            <tr>
                ...
                <th>Jumlah LK</th>
                <th>Jumlah PR</th>
                <th>Jumlah Keluar Hidup</th>
                <th>Jumlah Keluar Mati</th>
                <th>Total Pasien</th>
            </tr>
        </thead>
        <tbody>
...
<script type="text/javascript">
    ...
    $('.btn-get-data').click(function(){
        ...
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rawat-inap-laporan-10-besar-penyakit-per-bangsal/get-total-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
            ...
            success:function(results){
                ...
                array_10 = results.array_10
                jumlah_array_10 = results.jumlah_array_10
                var text_total_data = numeral(total_data).format('0,0');
                var text_total_data = text_total_data.replace(",",".")
                var text_jumlah_array_10 = numeral(jumlah_array_10).format('0,0');
                var text_jumlah_array_10 = text_jumlah_array_10.replace(",",".")
                $('.progress-data-loader-container').show();
                $('.progress-data-loader-total-data').html(text_jumlah_array_10);
                data_fetched = 0;
                if(jumlah_array_10 > 0) 
                {
                    getDataset()
                }
                else
    ...
    function getDataset()
    {
        var dataTableObj = $('.js-dataTable-full').DataTable();
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rawat-inap-laporan-10-besar-penyakit-per-bangsal/get-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
            data : 'datestart='+start_date+'&dateend='+end_date+'&datafetched='+data_fetched+'&bangsal='+bangsal+'&dtd='+array_10[data_fetched],
            success:function(results){
                ...
                if(data_fetched < jumlah_array_10) 
                {
                    getDataset()
                    var percentage = Math.ceil(data_fetched / jumlah_array_10 * 100)
