...
@section('title')
RL 4 Penyakit Rawat Jalan (Sebab Luar) - Laporan - Administrasi & Rekam Medis
@endsection
...
@section('page-title')
RL 4 Penyakit Rawat Jalan (Sebab Luar)
@endsection
    ...
    <div class="row">
        ...
        <div class="col-4">
            <div class="form-group">
                <label for="penyedia">Pilih Poliklinik</label>
                <select class="js-select2 form-control input-poli" name="poli[]" placeholder="Pilih Poliklinik" multiple="multiple" style="width: 100%;">
                    <option value="-1">IGD</option>
                    @foreach($poli as $poliklinik)
                    <option value="{{$poliklinik->id}}">{{$poliklinik->name}}</option>
                    @endforeach
                </select>
                <small>Kosongkan untuk melihat data dari semua ruangan poli & igd</small>
            </div>
        </div>
    ...
    <table class="table table-bordered table-striped table-vcenter js-dataTable-full" id="example">
        <thead>
            <tr>
                ...
                <th width="100px">Usia 0 - <=6hr (L)</th>
                <th width="100px">Usia 0 - <=6hr (P)</th>
                <th width="100px">Usia >6 - <=28hr (L)</th>
                <th width="100px">Usia >6 - <=28hr (P)</th>
                <th width="100px">Usia >28hr - <=1th (L)</th>
                <th width="100px">Usia >28hr - <=1th (P)</th>
                <th width="100px">Usia >1 - <=4th (L)</th>
                <th width="100px">Usia >1 - <=4th (P)</th>
                <th width="100px">Usia >4 - <=14-th (L)</th>
                <th width="100px">Usia >4 - <=14-th (P)</th>
                <th width="100px">Usia >14 - <=24th (L)</th>
                <th width="100px">Usia >14 - <=24th (P)</th>
                <th width="100px">Usia >24 - <=44th (L)</th>
                <th width="100px">Usia >24 - <=44th (P)</th>
                <th width="100px">Usia >44 - <=64th (L)</th>
                <th width="100px">Usia >44 - <=64th (P)</th>
                <th width="100px">Usia >64th (L)</th>
                <th width="100px">Usia >64th (P)</th>
                <th width="100px">KASUS BARU (L)</th>
                <th width="100px">KASUS BARU (P)</th>
                <th width="100px">JML KASUS BARU (L+P)</th>
...
<script type="text/javascript">
    ...
    $('.btn-get-data').click(function(){
        ...
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rl-4-penyakit-rawat-jalan/get-total-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
    ...
    function getDataset()
    {
        var dataTableObj = $('.js-dataTable-full').DataTable();
        $.ajax({ 
            url: API_URL + '/pasien/laporan-v2/page/rl-4-penyakit-rawat-jalan/get-data',
            dataType: 'json',
            tryCount : 0,
            retryLimit : 3,
            data : 'datestart='+start_date+'&dateend='+end_date+'&datafetched='+data_fetched+'&poli='+poli+'&dtd='+dtd[data_fetched],
            success:function(results){
