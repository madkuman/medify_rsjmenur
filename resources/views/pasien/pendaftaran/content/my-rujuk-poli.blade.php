    ...
    <div class="card-body mb-50">
        ...
        @forelse($my_rujuk_poli  as $item)
            @if (isset($item->kasus->active_sep->no_sep))
                <a class="block block-link-pop bg-danger-light text-danger" href="javascript:void(0)"
                    onclick="rujukClick('{{ $item->poli_tujuan->id }}','{{ $item->kasus->active_sep->no_sep }}', '{{ $item->kasus->active_sep->sisa_plafon }}', '{{ $item->kasus->id }}', '{{ $item->kasus->pasien_pembayaran_id }}','{{ $item->id }}','{{ $item->buat_kasus_baru }}' )">
                @else
                    <a class="block block-link-pop bg-danger-light text-danger" href="javascript:void(0)"
                        onclick="rujukClick('{{ $item->poli_tujuan->id }}',null,null, '{{ $item->kasus->id }}', '{{ $item->kasus->pasien_pembayaran_id }}','{{ $item->id }}','{{ $item->buat_kasus_baru }}' )">
            @endif
            <div class="block-content pb-10">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        @if ($item->buat_kasus_baru == 1)
                            Ya
                        @else($item->buat_kasus_baru == 2)
                            Tidak
                        @endif
                        <br>
