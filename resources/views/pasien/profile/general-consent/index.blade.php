...
<div class="btn-group float-right">
    ...
    <div class="dropdown-menu">
        <a class="dropdown-item" 
           href="{{ url('pasien/asesmen/general-consent') }}?pasien_id={{ $id }}&layouts=pasien">
           General Consent
        </a>
        <a class="dropdown-item" 
           href="{{ url('pasien/asesmen/general-consent-for-treatment') }}?pasien_id={{ $id }}&layouts=pasien">
...
<!--<h4>General Consent</h4>-->
<hr>
@php $treatmentcount = count($general_consent_treatment) @endphp
@forelse($general_consent_treatment as $item)
    @php $data_val = json_decode($item->val); @endphp
    @if($item->created_by == Auth::user()->id)
        <button  class="btn btn-sm btn-circle btn-outline-danger mr-5 mb-5 pull-right deleteBtn" data-id="{{$item->id}}">
            <i class="fa fa-trash"></i>
        </button>
        <a href="{{ url('') }}/pasien/asesmen/general-consent-for-treatment/form/edit?pasien_id={{$id}}&id={{$item->id}}" class="btn btn-sm btn-circle btn-outline-warning mr-5 mb-5 pull-right editBtn" data-id="{{$item->id}}" data-index="{{$loop->iteration - 1}}">
        <i class="fa fa-pencil"></i>
        </a>
    @if(empty($data_val->img_ttd))					
    <button  class="btn btn-sm btn-circle btn-outline-primary mr-5 mb-5 pull-right btn-add-ttd" data-toggle="modal" data-id="{{$item->id}}" data-pasien_id="{{$id}}" data-url="{{ url('') }}/pasien/asesmen/general-consent/add-ttd">
...
<a href="{{ url('') }}/pasien/asesmen/general-consent-for-treatment/single?pasien_id={{$id}}&id={{$item->id}}" class="btn btn-sm btn-circle btn-outline-primary mr-5 mb-5 pull-right showBtn" data-id="{{$item->id}}" data-index="{{$loop->iteration - 1}}">
    <i class="fa fa-search"></i>
</a>


<h5 class="mb-5 pl-5">#General Consent for Treatment {{$loop->iteration}}</h5>
...
@empty
    <div class="text-center py-50">
        ...
        <p>Klik tombol <b>General Consent for Treatment Baru</b> untuk melakukan asesmen</p>
    </div>
@endforelse
