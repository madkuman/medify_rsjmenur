    ...
    <main id="main-container">
        ...
        <div class="container">
            <div class="text-center">
                <h4 class="mb-5">Buat SEP Pasien</h4>
                            ...
                            <div class="block-content">
                                ...
                                <div class="py-20 row">
                                    ...
                                    <div class="form-group input-group col-10" id="sep_select_wrapper">
                                        <select name="sep" class="form-control js-select2" id="sep_select"
                                            data-placeholder="Nomor SEP Pasien" style="width: 80%;">
                                            <option value=""></option>
                                            @foreach ($sep as $item)
                                                @if (isset($item->no_sep))
                                                    <option value="{{ json_encode($item) }}"
                                                        @if (isset($transaksi->nomor_sep) && $transaksi->nomor_sep == $item->no_sep) selected="" @endif>
                                                        {{ $item->no_sep }} - @if ($item->jenis_pelayanan == 1)
                                                            Rawat Inap
                                                        @else
                                                            Rawat Jalan
                                                        @endif -
                                                        {{ !is_null($item->tgl_sep) ? indonesian_date(strtotime($item->tgl_sep)) : indonesian_date($item->created_at) }}
                                                    </option>
                                                @endif
                                            @endforeach
                                        </select>
                                        <div class="input-group-append" style="width: 20%;">
                                            ...
                                            <button type="button" class="btn btn-alt-primary" style="display: none;"
                                                id="sep_select_loading" disabled="">
                                    ...
                                    <div class="col-8" id="sep_manual_wrapper">
                                        <div class="form-group input-group">
                                            <label class="css-control css-control-primary css-checkbox">
                                                ...
                                                <span class="css-control-indicator"></span> Nomor SEP yang saya cari tidak
                                                terdaftar
                                            </label>
                                        </div>
                                        <div class="form-group" id="sep_custom_wrapper" style="display: none;">
                                            <label>Nomor SEP</label>
                                            <input type="text" name="custom_sep" id="custom_sep" class="form-control"
                                                placeholder="Nomor SEP Pasien" value="{{ $transaksi->nomor_sep }}">
                                        </div>
                                    </div>
            ...
            <div class="text-center mb-20">
                <a href="{{ url('rawatjalan/transaksi/pendaftaran') }}/{{ $transaksi->id }}"
                    class="btn btn-secondary">Kembali ke Sebelumnya</a>
            </div>
        </div>
    </main>
@endsection
...
@section('js')
    <script type="text/javascript">
        ...
        $('#sep_button').on('click', function(e) {
            ...
            popupwindow(
                "{{ url('') }}/bpjs/sep/create?window=true&pasien_id={{ $transaksi->pasien->id }}&pasien_name={{ $transaksi->pasien->name }}&rujukan={{ $transaksi->nomor_sep ?? '' }}",
                "Terbitkan SEP Baru", 900, 900);
        });
