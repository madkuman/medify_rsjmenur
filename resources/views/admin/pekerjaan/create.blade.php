...
@section('title')
Admin - Input Jenis Pekerjaan Baru
@endsection
	...
	<div class="block p-10">
		<div class="block-header">
			<h3 class="block-title">
				@if(empty($data))
				Input Jenis Pekerjaan Baru
				@else
				Edit Nama Jenis Pekerjaan
				@endif
			</h3>
		</div>
		<div class="block-content">
			<!--ketika dikoding ini diganti ya action sama methodnya -->
			<form action="{{url('admin/pekerjaan/simpan')}}" method="POST">
			{{csrf_field()}}
			@if(!empty($data))
				...
				<div class="row">
					<div class="col-6">
						<div class="form-group">
							...
							<input type="text" class="form-control" placeholder="Nama Pekerjaan" name="nama" 
							@if(!empty($data))
							value="{{$data->nama}}"
