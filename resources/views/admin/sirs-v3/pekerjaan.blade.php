			...
			<div class="row">
				<div class="col-12">
					...
					<table class="table table-bordered table-striped table-vcenter js-dataTable-full" id="show-data-table">
						...
						<tbody>
							@forelse ($pekerjaan_all as $item)
							<tr>
								<td class="text-center">{{ $loop->iteration }}</td>
								<td>{{ $item->nama }}</td>
								<td>{{ $pekerjaan_sirs[$item->sirs_pekerjaan_id] ?? '-' }}</td>
                    ...
                    <div class="modal-body px-0">
                        <div class="row">
							<div class="col-12">
								...
								<div class="form-group">
									<label>Jenis Pekerjaan SIRS</label>
									<select name="sirs_pekerjaan_id" class="js-select2" style="width: 100%">
										<option value="">Pilih Jenis Pekerjaan</option>
										@forelse ($pekerjaan_sirs as $id => $item)
...
<script>
var pekerjaan_all = @json($pekerjaan_all, JSON_PRETTY_PRINT);
$('.js-dataTable-full').dataTable();

$(document).on('click', '.btn-edit', function () {
	id = $(this).data('id');
	row = pekerjaan_all[id];
	$('#modal-edit').find('input[name=id]').val(id);
	$('#modal-edit').find('input[name=nama]').val(row.nama);
	$('#modal-edit').find('select[name=sirs_pekerjaan_id]').val(row.sirs_pekerjaan_id).trigger('change');
