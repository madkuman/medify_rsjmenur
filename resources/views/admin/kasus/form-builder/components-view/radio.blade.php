          ...
          <div class="custom-control custom-radio mb-5">
               ...
               <label class="custom-control-label" for="input-{{$opsi->id}}">
                   @if($checked && $show_data_as != 'edit') <strong class="text-black"> @endif
                        {{$opsi->deskripsi}} @if(!empty($input->hasil->keterangan) && $checked && $show_data_as == 'show') - ({{$input->hasil->keterangan}}) @endif
          ...
          <div class="form-group row" @if($opsi->extra_input!=1) hidden @endif>
               <div class="col-6">
                    <div class="form-material pt-0">
                         <input type="text" class="form-control" id="material-text" name="opsi-keterangan-{{$opsi->id}}" placeholder="Keterangan" 
                         value="@if(!empty($input->hasil->keterangan) && $checked) {{$input->hasil->keterangan}} @endif">
                    </div>
               </div>
          </div>
          @endif
