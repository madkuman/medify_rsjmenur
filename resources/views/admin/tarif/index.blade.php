					...
					<div class="row">
						<div class="col-3">
							...
							<select class="js-select2 form-control" id="kategori" name="kategori" style="width: 100%;">
								...
								<option value="{{$item->id}}" @if(isset($current_kategori) && $current_kategori==$item->id) selected @endif>{{$item->nama}}</option>
								@endforeach
							</select>
						</div>
				...
				<table class="table table-striped table-hover table-vcenter transaksiTable js-dataTable-full" id="transaksiTable">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a href="{{url()->current()}}/{{$item->id}}" class="btn btn-sm btn-alt-primary" data-toggle="tooltip" title="Lihat Detail ">
