	...
	<div class="block p-10">
		<div class="block-header">
			<h3 class="block-title">
				<small><a href="{{url('admin/lokasi-zona-ppi/baru')}}" class="pull-right">
				<i class="fa fa-plus-circle"></i> Input Lokasi Zona PPI Baru</a></small> 
				Daftar Lokasi  Zona PPI
			</h3>
		</div>
		<div class="block-content">
			<table class="table table-bordered table-striped table-vcenter js-dataTable-full" id="example">
				...
				@foreach($data as $isi)
					<tr>
						<td class="">{{$i}}</td>
						<td class="font-w600">{{$isi->zona}}</td>
						<td class="font-w600">{{$isi->deskripsi}}</td>
						<td class="">
							<a href="{{url('/admin/lokasi-zona-ppi/edit/')}}/{{$isi->id}}" class="btn btn-sm btn-circle btn-outline-info mr-5 mb-5">
...
<script type="text/javascript">
    ...
    $(document).on("click",".btn-outline-danger", function () {
        ...
        console.log(id,nama);
        $("#del-btn").attr('href','{{url('admin/lokasi-zona-ppi/delete')}}' + '/' + id)
