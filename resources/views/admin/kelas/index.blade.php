	...
	<div class="block p-10">
		<div class="block-header">
			<h3 class="block-title">
				<small><a href="{{url('admin/kelas/baru')}}" class="pull-right">
				<i class="fa fa-plus-circle"></i> Input Kelas Baru</a></small> 
				Daftar Kelas
			</h3>
		</div>
			...
			<table class="table table-bordered table-striped table-vcenter js-dataTable-full" id="example">
				...
				@foreach($data as $isi)
					<tr>
						...
						<td class="font-w600">
							@if($isi->rawat_inap)<span class="badge badge-primary">Rawat Inap</span>@endif
							@if($isi->rawat_jalan)<span class="badge badge-success">Rawat Jalan</span>@endif
							@if($isi->igd)<span class="badge badge-danger">IGD</span>@endif
							@if($isi->medical_checkup)<span class="badge badge-info">Medical Checkup</span>@endif
						</td>
						<td class="">
							<a href="{{url('/admin/kelas/edit/')}}/{{$isi->id}}" class="btn btn-sm btn-circle btn-outline-info mr-5 mb-5">
...
<script type="text/javascript">
    ...
    $(document).on("click",".btn-outline-danger", function () {
        ...
        console.log(id,nama);
        $("#del-btn").attr('href','{{url('admin/kelas/delete')}}' + '/' + id)
