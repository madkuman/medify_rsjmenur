...
@section('title')
Admin - Input SIRS Kegiatan Radiologi Baru
@endsection
	...
	<div class="block p-10">
		<div class="block-header">
			<h3 class="block-title">
				@if(empty($data))
				Input SIRS - Kegiatan Radiologi Baru
				@else
				Edit SIRS - Kegiatan Radiologi
				@endif
			</h3>
		</div>
		<div class="block-content">
			<!--ketika dikoding ini diganti ya action sama methodnya -->
			<form action="{{url('admin/sirs-kegiatan-radiologi/simpan')}}" method="POST">
			{{csrf_field()}}
			@if(!empty($data))
...
<script type="text/javascript">
	...
	function deleteModal(id)
	{
		swal({
			...
			text: "Kegiatan Radiologi akan terhapus dari daftar",
			type: 'warning',
    ...
    $(document).on("click",".btn-outline-danger", function () {
        ...
        console.log(id,nama);
        $("#del-btn").attr('href','{{url('sirs-kegiatan-radiologi/delete')}}' + '/' + id)
