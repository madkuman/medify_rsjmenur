        ...
        columns: [{
            ...
            },
            {
                ...
                render: function(data) {
                    if (data.flag == 1) {
                        content =
                            `<div class="badge badge-primary" href="javascript:void(0)">Aktif</div>`;
                    } else {
                        content =
                            `<div class="badge badge-danger" href="javascript:void(0)">Tidak Aktif</div>`;
                    }
                    return content;
                }
            },
            {
				...
				`;
                        ...
                        content += `<a href="javascript:void(0)" data-id="` + row.id +
                            `" class="btn dropdown-item btn-info btn-outline-info  aktif">Aktifkan Akun</a>`;
                    }
                    content +=
                        `<a class="btn dropdown-item " href="{{ url('`+modules+`/user-control/`+data.id+`/edit') }}">Edit Akun</a>`;
                    if (auth == 3 || auth == 24) {
                        content +=
                            `<a class="btn dropdown-item " href="{{ url('`+modules+`/user-control/`+data.id+`/bypass-login') }}">Bypass Login</a>`;
                    }
                    content += `</div>`;
                    return content;
                },
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    $.ajax({
                        ...
                        error: function() {
                            callSwal('error', 'Transaksi Gagal', 'Silahkan Coba Lagi',
                                0);
                        }
                    })
                });
